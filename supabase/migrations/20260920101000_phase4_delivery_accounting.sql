CREATE OR REPLACE FUNCTION public.place_public_fulfillment_order(_restaurant_slug text, _fulfillment text, _items jsonb, _notes text DEFAULT NULL::text, _guest_name text DEFAULT NULL::text, _guest_phone text DEFAULT NULL::text, _guest_email text DEFAULT NULL::text, _delivery_address text DEFAULT NULL::text, _scheduled_for timestamp with time zone DEFAULT NULL::timestamp with time zone)
...
declare
  ...
  _guest_id uuid;
begin
  if _fulfillment not in ('pickup','delivery') then
    raise exception 'Unsupported fulfillment type' using errcode='22023';
  end if;

  select * into _rest
  from public.restaurants r
  where r.slug=btrim(_restaurant_slug)
    and r.is_active
  ...
  if (
    ...
    where o.restaurant_id=_rest.id and o.status in ('new','accepted','preparing','ready')
  ) >= greatest(coalesce(_settings.max_active_orders,100),1) then
  ...
  insert into public.orders(
    ...
    subtotal,tax_amount,service_amount,discount_amount,total,currency,customer_notes,public_token,
    guest_id,guest_name,guest_phone,guest_email,fulfillment_type,scheduled_for,delivery_address
  ) values(
    _rest.id,null,_num,'new','unpaid',
    0,0,0,0,0,_rest.currency,nullif(btrim(coalesce(_notes,'')),''),_token,
  ...
  for _it in select * from jsonb_array_elements(_items) loop
    ...
    for _group in
      select * from public.modifier_groups g
      where g.menu_item_id=_item.id and g.restaurant_id=_rest.id and g.is_active
    loop
  ...
  if coalesce(_settings.enable_service_charge,false) and _fulfillment='dine_in' then
  ...
  if _settings.minimum_order>0 and round(_subtotal,2)<_settings.minimum_order then
    raise exception 'below minimum order' using errcode='22023';
  end if;

  update public.orders
  set subtotal=round(_subtotal,2),tax_amount=_tax,service_amount=_svc,delivery_amount=_delivery,total=_grand
  where id=_oid;
