...
export function PdfMenuViewer({
  ...
  const pageLinks = useMemo(
    () =>
      links.filter(
        (link) => link.page_number === page && itemById.has(link.menu_item_id),
      ),
    [links, page, itemById],
  );
    ...
    async function loadDocument() {
      ...
      try {
        ...
        if (canvasRef.current) {
          await renderPdfPage(pdf, actualPage, canvasRef.current, 1600);
        }
        if (actualPage < pdf.numPages) {
          void pdf.getPage(actualPage + 1);
        }
      } catch (reason) {
        if (!disposed) {
          setError(
    ...
    <section className="mx-auto w-full max-w-4xl px-0 pb-24 sm:px-4">
      ...
      <div
        ...
        onTouchMove={(event) => {
          ...
          const next = Math.min(
            3,
            Math.max(1, start.zoom * (distance / start.distance)),
          );
          setZoom(next);
        }}
        onTouchEnd={(event) => {
