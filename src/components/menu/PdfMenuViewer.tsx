...
export function PdfMenuViewer({ url, parts = [], pageCount, links, items, onSelect, cartCount, onCart, showCart }: {
  ...
  useEffect(() => {
    ...
    async function loadDocument() {
      ...
      try {
        ...
        if (actualPage !== page) { setPage(actualPage); return; }
  ...
  return <section className="mx-auto w-full max-w-4xl px-0 pb-24 sm:px-4">
    <div className="sticky top-0 z-20 mb-2 flex items-center justify-between gap-2 border-b bg-background/95 px-2 py-2 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-1">
        <Button size="icon" variant="ghost" disabled={zoom <= 1} onClick={() => changeZoom(-0.25)} aria-label="Zoom out"><ZoomOut className="size-4"/></Button>
        <span className="w-10 text-center text-[11px] font-semibold tabular-nums">{Math.round(zoom * 100)}%</span>
        <Button size="icon" variant="ghost" disabled={zoom >= 3} onClick={() => changeZoom(0.25)} aria-label="Zoom in"><ZoomIn className="size-4"/></Button>
    ...
    <div
      ...
      onTouchMove={(event) => {
        ...
        const next = Math.min(3, Math.max(1, start.zoom * (distance / start.distance)));
        setZoom(next);
      }}
      onTouchEnd={(event) => {
    ...
    >
      <div className="relative mx-auto w-full origin-top" style={{ transform: `scale(${zoom})`, transformOrigin: "top center", marginBottom: `${Math.max(0, (zoom - 1) * 100)}%` }}>
