...
export function PdfMenuViewer({
  url, parts = [], pageCount, links, items, onSelect, cartCount, onCart, showCart,
}: {
  url: string; parts?: string[]; pageCount: number; links: PdfLink[]; items: DinerItem[];
  onSelect: (item: DinerItem) => void; cartCount: number; onCart: () => void; showCart: boolean;
}) {
  ...
  useEffect(() => { setPage((value) => Math.min(Math.max(1, value), safePageCount)); }, [safePageCount]);
  ...
  useEffect(() => {
    ...
    async function loadDocument() {
      ...
      try {
        ...
        if (disposed) return;
        pdfRef.current = pdf;
        if (page > pdf.numPages) { setPage(pdf.numPages); return; }
        if (canvasRef.current) await renderPdfPage(pdf, page, canvasRef.current, 1400);
        if (page < pdf.numPages) void pdf.getPage(page + 1);
  ...
  return <section className="mx-auto w-full max-w-4xl px-2 pb-24 sm:px-4">
    <div className="sticky top-0 z-20 mb-3 flex items-center justify-between gap-2 border-b bg-background/90 px-1 py-2 backdrop-blur-xl">
      <div className="flex items-center gap-2"><Button size="icon" variant="outline" disabled={page <= 1 || loading} onClick={() => go(-1)} aria-label="Previous page"><Minus className="size-4"/></Button><span className="min-w-20 text-center text-xs font-semibold tabular-nums">Page {page} / {safePageCount}</span><Button size="icon" variant="outline" disabled={page >= safePageCount || loading} onClick={() => go(1)} aria-label="Next page"><Plus className="size-4"/></Button></div>
      {showCart && cartCount > 0 ? <Button size="sm" onClick={onCart}><ShoppingBag className="size-4"/> Cart ({cartCount})</Button> : null}
    </div>
    <div className="relative mx-auto w-full overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-black/5 touch-pan-y"
      onTouchStart={(event) => { touchStartX.current = event.changedTouches[0]?.clientX ?? null; }}
      onTouchEnd={(event) => { const start = touchStartX.current; touchStartX.current = null; const end = event.changedTouches[0]?.clientX; if (start == null || end == null || Math.abs(end - start) < 55 || loading) return; go(end < start ? 1 : -1); }}>
