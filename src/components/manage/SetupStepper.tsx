...
/** Guided onboarding for a freshly created restaurant. The former Design step now points to the PDF Menu page. */
export function SetupStepper({ restaurantId }: { restaurantId: string }) {
  ...
  return <section className="panel p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-sm font-semibold">{t("onboard.title")}</h2><p className="mt-1 text-xs text-muted-foreground">{t("onboard.subtitle")}</p></div><Button asChild size="sm" className="h-9"><Link to={next.to} params={{ restaurantId }}>{t("onboard.continue")}<ChevronRight className="size-4" /></Link></Button></div><div className="mt-4 flex items-center gap-3"><Progress value={percent} className="h-2 flex-1"/><span className="shrink-0 text-xs font-semibold tabular-nums text-muted-foreground">{percent}% {t("onboard.progress")}</span></div><ol className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{steps.map((s, i) => <li key={s.key}><Link to={s.to} params={{ restaurantId }} className="flex items-start gap-3 rounded-xl border border-border p-3 transition-colors hover:bg-muted/60"><span className={`mt-0.5 grid size-6 shrink-0 place-items-center rounded-full text-[11px] font-semibold ${s.done ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{s.done ? <Check className="size-3.5"/> : i + 1}</span><span className="min-w-0"><span className="block truncate text-sm font-medium">{s.label}</span><span className="block text-xs text-muted-foreground">{s.help}</span></span></Link></li>)}</ol></section>;
}
