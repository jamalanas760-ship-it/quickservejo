...
function MiniArtboard({ theme, restaurantName, selected }: { theme: StudioDesign; restaurantName: string; selected?: boolean }) {
  ...
  return (
    <div
      className={cn(
        "relative aspect-[4/5] overflow-hidden rounded-[22px] border shadow-[0_18px_50px_rgba(0,0,0,.12)] transition-transform",
        selected && "ring-2 ring-black ring-offset-2",
      )}
      style={{ background: theme.bg, color: theme.text, fontFamily: FONT_STACKS[theme.bodyFont] }}
    >
      ...
      <div className="relative p-5">
        ...
        <div className="mt-5 flex items-end justify-between gap-3">
          <div><div className="text-[9px] font-bold uppercase tracking-[.15em]" style={{ color: theme.accent }}>Signature</div><div className="mt-1 text-sm font-bold">Chef's selection</div><div className="mt-1 text-[9px]" style={{ color: theme.muted }}>Prepared fresh with care.</div></div>
          <div className="text-sm font-black" style={{ color: theme.primary }}>8.50 JOD</div>
        </div>
...
export function MasterMenuDesigner({ restaurantId }: { restaurantId: string }) {
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] overflow-hidden rounded-[32px] border bg-[#f6f5f2] text-black shadow-[0_20px_70px_rgba(0,0,0,.08)]">
      <div className="border-b bg-white/90 px-4 py-3 backdrop-blur-xl sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-2xl bg-black text-white"><Sparkles className="size-5" /></div><div className="min-w-0"><div className="flex items-center gap-2"><h1 className="truncate text-base font-black tracking-tight sm:text-lg">Master Menu Designer</h1><Badge className="rounded-full bg-black text-white">AI Art Studio</Badge></div><p className="truncate text-xs text-black/45">{currentRestaurant.name} · changes preview instantly before saving</p></div></div>
          <div className="flex items-center gap-2"><Badge variant="outline" className={cn("rounded-full", lastSynced && "border-emerald-300 text-emerald-700")}><span className={cn("mr-1.5 inline-block size-1.5 rounded-full bg-amber-400", lastSynced && "bg-emerald-500")} />{lastSynced ? "Live menu synced" : "Draft preview"}</Badge><Button onClick={save} disabled={saving} className="rounded-xl bg-black text-white hover:bg-black/85"><Save className="size-4" /> {saving ? "Saving…" : "Publish design"}</Button></div>
        </div>
      </div>

      <div className="grid min-h-[760px] lg:grid-cols-[350px_minmax(0,1fr)_310px]">
        <aside className="order-2 border-r bg-[#faf9f6] lg:order-1">
          <div className="flex border-b bg-white">{(["create", "art", "edit"] as Direction[]).map((tab) => <button key={tab} type="button" onClick={() => setActiveSection(tab)} className={cn("flex-1 px-3 py-3 text-xs font-bold capitalize", activeSection === tab ? "border-b-2 border-black text-black" : "text-black/40")}>{tab === "create" ? "Create" : tab === "art" ? "Art direction" : "Edit"}</button>)}</div>
          <div className="max-h-[calc(100vh-250px)] space-y-5 overflow-y-auto p-4">
            {activeSection === "create" && <>
              <section><div className="mb-2 flex items-center justify-between"><h2 className="text-xs font-black uppercase tracking-[.16em] text-black/45">Creative brief</h2><span className="text-[10px] text-black/35">{brief.length}/6000</span></div><Textarea value={brief} onChange={(e) => setBrief(e.target.value)} placeholder="Describe the restaurant mood, cuisine, audience, Arabic/English preference, reference style, photography and what should feel different." className="min-h-28 resize-none rounded-2xl border-black/10 bg-white text-sm" /></section>
            ...
            {activeSection === "edit" && <>
              <section><div className="mb-2 text-xs font-black uppercase tracking-[.16em] text-black/45">AI actions</div><div className="grid grid-cols-1 gap-2">{ART_ACTIONS.map(([label, instruction]) => <button key={label} type="button" disabled={busy} onClick={() => void generateDesign(instruction)} className="rounded-xl border border-black/10 bg-white px-3 py-2.5 text-left text-xs font-semibold hover:border-black/25 disabled:opacity-50"><Sparkles className="mr-2 inline size-3.5" />{label}</button>)}</div></section>
