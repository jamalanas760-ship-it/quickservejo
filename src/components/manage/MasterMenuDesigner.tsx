...
export function MenuDesigner({ restaurantId }: { restaurantId: string }) {
  ...
  const currentElements = useMemo(() => { const responsiveElements = theme.composition?.[previewMode]?.elements; return responsiveElements?.length ? responsiveElements : theme.composition?.elements ?? derivedComposition.elements; }, [theme.composition, previewMode, derivedComposition.elements]);
  ...
  function applyConcept(id: string) {
    ...
    setConcept(id); setDirection(id === "poster" ? "poster" : id === "cinematic" ? "luxe" : "editorial"); setSelectedId("image-main");
  ...
  function applyDirection(id: string) {
    ...
    setDirection(id); setTheme({ ...nextTheme, composition: nextComposition });
  }
  ...
  async function addReferences(files: FileList | null) {
    ...
    try {
      ...
      const encoded = await Promise.all(selectedFiles.map(async (file) => {
        const bitmap = await createImageBitmap(file); const max = 1400; const scale = Math.min(1, max / Math.max(bitmap.width, bitmap.height));
        const canvas = document.createElement("canvas"); canvas.width = Math.max(1, Math.round(bitmap.width * scale)); canvas.height = Math.max(1, Math.round(bitmap.height * scale));
        const ctx = canvas.getContext("2d"); if (!ctx) throw new Error("Could not read image"); ctx.drawImage(bitmap, 0, 0, canvas.width, canvas.height); bitmap.close();
        return canvas.toDataURL("image/jpeg", 0.88);
      }));
      setReferences((current) => [...current, ...encoded].slice(0, 5)); toast.success(lang === "ar" ? "تم إرفاق الصور مع وصف الذكاء الاصطناعي" : "References attached to the AI brief");
  ...
  async function generateDesign() {
    ...
    try {
      ...
      const variants = result.variants.map((value) => { const raw = JSON.parse(value) as DesignerTheme; const parsed = parseMenuTheme(raw); return { ...parsed, composition: normalizeComposition(raw.composition, parsed, restaurant.data?.name ?? "Restaurant", references, concept, menuItems) } as DesignerTheme; });
      if (variants[0]) setTheme(variants[0]); if (variants[0]?.composition) setSelectedId(variants[0].composition.elements[0]?.id ?? "image-main");
      toast.success(lang === "ar" ? "تم إنشاء التصميم وتحديث المعاينة مباشرة" : "Master design generated and preview updated");
    } catch (error) { toast.error(humanError(error, lang)); } finally { setGenerating(false); }
  }

  async function save() {
    setSaving(true);
    try {
      const { error } = await supabase.from("restaurants").update({ menu_theme: theme as unknown as never }).eq("id", restaurantId); if (error) throw error;
      ...
      await queryClient.invalidateQueries({ queryKey: ["master-menu-designer", restaurantId] }); toast.success(t("common.saved"));
  ...
  return <div className="min-h-screen bg-[#f4f4f2] text-[#171716]"><div className="mx-auto max-w-[1760px] p-3 sm:p-5 lg:p-6">
    <header className="sticky top-2 z-50 mb-4 flex flex-wrap items-center gap-3 rounded-[1.5rem] border border-black/[0.07] bg-white/92 p-3 shadow-[0_12px_40px_rgba(0,0,0,.08)] backdrop-blur-xl sm:p-4"><div className="grid size-10 shrink-0 place-items-center rounded-xl bg-black text-white sm:size-11 sm:rounded-2xl"><Wand2 className="size-5" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h1 className="truncate text-base font-bold tracking-tight sm:text-lg">Master Menu Studio</h1><Badge className="hidden bg-orange-500 text-white sm:inline-flex">AI + DESIGN</Badge></div><p className="truncate text-xs text-black/45">{restaurant.data.name} · live composition editor</p></div><div className="flex w-full gap-2 sm:w-auto"><Button variant="outline" size="sm" className="flex-1 sm:flex-none" asChild><a href={`/r/${restaurant.data.slug}`} target="_blank" rel="noreferrer"><ExternalLink className="size-4" />Open menu</a></Button><Button size="sm" className="flex-1 sm:flex-none" disabled={saving} onClick={() => void save()}><Save className="size-4" />{saving ? "Saving…" : "Save design"}</Button></div></header>
    <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)_330px] xl:items-start">
      <aside className="space-y-3 xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto xl:pr-1">
        ...
        <Panel title="AI menu director" icon={<Sparkles className="size-4" />}><div className="mb-2 flex gap-1.5 overflow-x-auto pb-1">{PROVIDER_LABELS.map(([id, label]) => <button key={id} type="button" onClick={() => setPromptProvider(id)} className={cn("shrink-0 rounded-full border px-2.5 py-1.5 text-[9px] font-semibold", promptProvider === id ? "border-black bg-black text-white" : "border-black/10 bg-white text-black/55")}>{label}</button>)}</div><div className="relative rounded-2xl border border-black/10 bg-[#fafaf8] p-2"><Textarea value={brief} onChange={(e) => setBrief(e.target.value)} rows={5} className="resize-none border-0 bg-transparent pr-8 text-xs shadow-none focus-visible:ring-0" placeholder="Describe the menu you want… The attached photos are part of this AI brief." /><button type="button" onClick={() => fileRef.current?.click()} className="absolute bottom-3 start-3 grid size-8 place-items-center rounded-xl border bg-white text-black/55 shadow-sm hover:text-black" aria-label="Attach reference image"><Paperclip className="size-4" /></button><input ref={fileRef} type="file" accept="image/*" multiple className="hidden" onChange={(e) => { void addReferences(e.target.files); e.currentTarget.value = ""; }} /><button type="button" onClick={() => void copyPrompt()} className="absolute bottom-3 end-3 grid size-8 place-items-center rounded-xl border bg-white text-black/55 shadow-sm hover:text-black" aria-label="Copy AI prompt"><Copy className="size-4" /></button></div>{references.length > 0 && <div className="mt-2 flex items-center gap-2 rounded-xl border bg-[#fafaf8] p-2"><div className="flex -space-x-2">{references.map((src, i) => <div key={`${src.slice(-12)}-${i}`} className="relative"><img src={src} alt="Attached reference" className="size-9 rounded-lg border-2 border-white object-cover" /><button type="button" onClick={() => setReferences((current) => current.filter((_, index) => index !== i))} className="absolute -end-1.5 -top-1.5 grid size-4 place-items-center rounded-full bg-black text-white"><X className="size-2.5" /></button></div>)}</div><span className="text-[9px] text-black/45">{references.length} visual reference{references.length > 1 ? "s" : ""} attached to the prompt</span></div>}<div className="mt-2 flex gap-2"><Button className="h-10 flex-1 rounded-xl" disabled={generating} onClick={() => void generateDesign()}><Sparkles className="size-4" />{generating ? "Designing…" : "Generate master design"}</Button><Button variant="outline" className="h-10 rounded-xl px-3" onClick={() => void copyPrompt()}><Copy className="size-4" /></Button></div></Panel>
      ...
      <main className="min-w-0 rounded-[1.5rem] border border-black/[0.07] bg-[#dededb] p-3 shadow-sm sm:p-4"><div className="mb-3 flex flex-wrap items-center gap-2 rounded-2xl border border-black/[0.06] bg-white p-2"><div className="flex rounded-xl bg-[#f5f5f3] p-1">{(["phone", "tablet", "desktop"] as PreviewMode[]).map((mode) => <button key={mode} type="button" onClick={() => setPreviewMode(mode)} className={cn("grid size-8 place-items-center rounded-lg text-black/45", previewMode === mode && "bg-black text-white")}>{mode === "phone" ? <Smartphone className="size-4" /> : mode === "tablet" ? <Tablet className="size-4" /> : <Monitor className="size-4" />}</button>)}</div><div className="ml-auto flex items-center gap-1"><button type="button" onClick={() => setZoom((value) => Math.max(55, value - 5))} className="grid size-8 place-items-center rounded-lg hover:bg-black/5"><ZoomOut className="size-4" /></button><span className="w-10 text-center text-[10px] font-semibold text-black/45">{zoom}%</span><button type="button" onClick={() => setZoom((value) => Math.min(110, value + 5))} className="grid size-8 place-items-center rounded-lg hover:bg-black/5"><ZoomIn className="size-4" /></button><button type="button" onClick={() => setGrid((value) => !value)} className={cn("ml-1 grid size-8 place-items-center rounded-lg", grid ? "bg-black text-white" : "hover:bg-black/5")}><Grid3X3 className="size-4" /></button></div></div><div className="flex min-h-[720px] items-start justify-center overflow-auto rounded-2xl bg-[#cfcfcb] p-5 sm:p-8"><div className="relative shrink-0 shadow-[0_24px_70px_rgba(0,0,0,.18)]" style={{ width: `${previewWidth}px`, maxWidth: "100%", transform: `scale(${zoom / 100})`, transformOrigin: "top center", marginBottom: `${Math.max(0, (zoom / 100 - 1) * 500)}px` }}><div className="relative overflow-hidden rounded-[22px]" style={{ ...themeVars(theme), aspectRatio: `${previewRatio}`, background: theme.bg, color: theme.text, fontFamily: "var(--qs-body-font)" }}>{grid && <div className="pointer-events-none absolute inset-0 z-30" style={{ backgroundImage: "linear-gradient(to right, rgba(0,0,0,.08) 1px, transparent 1px), linear-gradient(to bottom, rgba(0,0,0,.08) 1px, transparent 1px)", backgroundSize: "10% 10%" }} />}<div className="absolute inset-0 opacity-60" style={{ background: theme.bgStyle === "glow" ? `radial-gradient(circle at 75% 10%, ${theme.accent}22, transparent 35%)` : theme.bgStyle === "gradient" ? `linear-gradient(145deg, ${theme.bg}, ${theme.surface})` : undefined }} />{theme.texture !== "none" && <div className="pointer-events-none absolute inset-0 opacity-[.08]" style={{ backgroundImage: theme.texture === "paper" ? "radial-gradient(#000 0.6px, transparent 0.7px)" : theme.texture === "chalk" ? "radial-gradient(#fff 0.5px, transparent 0.7px)" : "radial-gradient(#000 0.5px, transparent 0.8px)", backgroundSize: "7px 7px" }} />}{currentElements.map((element) => { const isSelected = selectedId === element.id; const style: CSSProperties = { left: `${element.x}%`, top: `${element.y}%`, width: `${element.w}%`, height: `${element.h}%`, color: element.color || theme.text, opacity: element.opacity ?? 1, transform: `rotate(${element.rotation ?? 0}deg)`, zIndex: isSelected ? 20 : 10 }; if (element.type === "image") return <button key={element.id} type="button" onClick={() => setSelectedId(element.id)} className={cn("absolute overflow-hidden transition-[box-shadow,transform]", motionClass, isSelected && "ring-2 ring-[var(--qs-primary)] ring-offset-2 ring-offset-transparent")} style={{ ...style, borderRadius: element.shape === "circle" ? "999px" : element.shape === "organic" ? "36% 64% 62% 38% / 42% 38% 62% 58%" : element.shape === "square" ? "0" : `${theme.radius}px` }}><img src={element.image || FOOD[0]} alt="" className="size-full object-cover" /></button>; const fontSize = Math.max(8, (element.fontSize || 16) * (previewMode === "phone" ? 0.68 : previewMode === "tablet" ? 0.86 : 1)); return <button key={element.id} type="button" onClick={() => setSelectedId(element.id)} className={cn("absolute whitespace-pre-line overflow-hidden text-start transition-[box-shadow]", isSelected && "rounded-md ring-1 ring-[var(--qs-primary)] ring-offset-2 ring-offset-transparent")} style={{ ...style, fontFamily: element.type === "title" || element.type === "brand" ? "var(--qs-heading-font)" : "var(--qs-body-font)", fontSize, fontWeight: element.type === "title" ? 700 : element.type === "product" || element.type === "price" ? 650 : 450, textAlign: element.align || "left", lineHeight: 1.05 }}>{element.text}</button>; })}<div className="pointer-events-none absolute bottom-3 end-3 rounded-full bg-black/10 px-2 py-1 text-[7px] font-semibold uppercase tracking-[.15em]" style={{ color: theme.muted }}>Live preview</div></div></div></div><div className="mt-3 flex items-center justify-center gap-2 text-[10px] text-black/40"><Eye className="size-3.5" /> Changes reflect instantly in the canvas</div></main>
      <aside className="min-w-0 rounded-[1.5rem] border border-black/[0.07] bg-white shadow-sm xl:sticky xl:top-24 xl:max-h-[calc(100vh-7rem)] xl:overflow-y-auto"><div className="grid grid-cols-4 border-b border-black/[0.07]">{([['style','Style'],['layout','Layout'],['type','Typography'],['motion','Motion']] as Array<[EditorTab,string]>).map(([id,label]) => <button key={id} type="button" onClick={() => setTab(id)} className={cn("h-12 text-[11px] font-medium text-black/45", tab === id && "bg-black text-white")}>{label}</button>)}</div><div className="space-y-5 p-4">
