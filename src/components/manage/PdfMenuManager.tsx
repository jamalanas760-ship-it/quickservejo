...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (value: string) => void; type?: "text" | "number" }) { return <div><label className="text-xs font-medium text-muted-foreground">{label}</label><Input type={type} className="mt-1.5" value={value} onChange={(e) => onChange(e.target.value)}/></div>; }
...
function PdfOverlayPreview({ canvasRef, candidates, drafts, onSelect }: { canvasRef: RefObject<HTMLCanvasElement | null>; candidates: PdfMenuCandidate[]; drafts: Record<string, Draft>; onSelect: (candidate: PdfMenuCandidate) => void }) { return <div className="relative mx-auto w-full max-w-3xl overflow-hidden rounded-xl bg-white shadow-sm"><canvas ref={canvasRef} className="block h-auto w-full"/><div className="absolute inset-0">{candidates.map((candidate) => { const enabled = drafts[candidate.id]?.enabled; return <button key={candidate.id} type="button" aria-label={`${enabled ? "Clickable" : "Not clickable"} ${candidate.id}`} onClick={() => onSelect(candidate)} className={`absolute rounded-md border-2 transition ${enabled ? "border-emerald-500 bg-emerald-500/10" : "border-transparent bg-transparent hover:border-primary/50 hover:bg-primary/5"}`} style={{ left: `${candidate.x * 100}%`, top: `${candidate.y * 100}%`, width: `${candidate.width * 100}%`, height: `${candidate.height * 100}%` }}><span className="sr-only">{candidate.id}</span></button>; })}</div></div>; }
