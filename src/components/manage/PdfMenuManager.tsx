...
export function PdfMenuManager({ restaurantId }: { restaurantId: string }) {
  ...
  async function handleFile(nextFile?: File) {
    if (!nextFile) return;
    if (nextFile.type !== "application/pdf" && !nextFile.name.toLowerCase().endsWith(".pdf")) return toast.error("Please upload a PDF menu.");
    if (nextFile.size > MAX_PDF_BYTES) return toast.error("PDF is too large. Maximum allowed size is 100 MB.");
    setBusy(true);
    try {
  ...
  async function saveSelectedProduct() {
    ...
    if (!nameEn) return toast.error("Product title is required.");
    setBusy(true);
    try {
...
function PdfOverlayPreview({ canvasRef, candidates, drafts, selecting, onManualSelect, onOpenCandidate }: { canvasRef: RefObject<HTMLCanvasElement | null>; candidates: PdfMenuCandidate[]; drafts: Record<string, Product>; selecting: boolean; onManualSelect: (rect: SelectionRect) => void; onOpenCandidate: (candidate: PdfMenuCandidate) => void }) {
  ...
  function up(event: React.PointerEvent<HTMLDivElement>) { const drag = dragRef.current; if (!selecting || !drag || drag.pointerId !== event.pointerId) return; dragRef.current = null; const p = point(event); const rect = { x: Math.min(drag.startX, p.x), y: Math.min(drag.startY, p.y), width: Math.abs(p.x - drag.startX), height: Math.abs(p.y - drag.startY) }; setSelection(null); if (rect.width < 0.01 || rect.height < 0.01) return toast.info("Drag around the full product area."); onManualSelect(rect); }
