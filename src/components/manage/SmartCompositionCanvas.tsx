 ...
 return <div className="smart-menu-canvas mx-auto w-full max-w-[960px] overflow-hidden rounded-[24px] shadow-2xl" style={{background:composition?.background?.color??theme.bg,color:theme.text}}>
  <div className="smart-menu-canvas-surface relative aspect-[4/3] w-full overflow-hidden">
   ...
   {elements.map(el=>{
    ...
    const left=clamp(el.x,0,100);
    const top=clamp(el.y,0,100);
    const width=clamp(el.w,1,100-left);
    const height=clamp(el.h,1,100-top);
    const safeTextWidth=isText && width<16 ? Math.min(38,100-left) : width;
    const textSize=el.fontSize ? clamp(el.fontSize,10,72) : undefined;
    return <button key={el.id} type="button" aria-label={`Edit ${el.type}`} onClick={()=>onSelect(el.id)} className={`smart-menu-element absolute overflow-hidden border-0 bg-transparent p-0 text-left ${selected?"ring-2 ring-offset-1 ring-black/60":"hover:ring-1 hover:ring-black/20"}`} style={{left:`${left}%`,top:`${top}%`,width:`${safeTextWidth}%`,height:`${height}%`,zIndex:el.z??0,transform:`rotate(${el.rotation??0}deg)`,opacity:el.opacity??1,borderRadius:shape==="circle"?"999px":shape==="rounded"?`${Math.max(8,theme.radius)}px`:shape==="organic"?"34% 66% 48% 52% / 58% 42% 58% 42%":"10px",background:el.color??(el.type==="image"?`linear-gradient(135deg, ${theme.primary}55, ${theme.accent}44)`:"transparent"),animation:animation!=="none"?`${animation} 900ms ease both`:undefined}}>
      {el.type==="image"?<div className="h-full w-full"><img src={el.image??""} alt="" className="h-full w-full object-cover" onError={(event)=>{event.currentTarget.style.display="none"}}/>{!el.image&&<div className="grid h-full w-full place-items-center bg-black/5"><Camera className="size-7 opacity-20"/></div>}</div>:el.type==="shape"?<div className="h-full w-full"/>:<div className="flex h-full w-full min-w-0 items-center px-2 sm:px-3" style={{justifyContent:el.align==="center"?"center":el.align==="right"?"flex-end":"flex-start",textAlign:el.align??"left"}}><span className="block max-w-full overflow-hidden text-ellipsis" style={{fontSize:textSize?`clamp(10px, ${Math.min(textSize,48)}px, ${textSize}px)`:undefined,fontFamily:el.fontFamily,color:el.color&&el.type!=="shape"?el.color:undefined,fontWeight:el.fontWeight,letterSpacing:el.letterSpacing!==undefined?`${clamp(el.letterSpacing,-2,12)}px`:undefined,lineHeight:el.lineHeight?clamp(el.lineHeight,.8,2):1.15,whiteSpace:"normal",overflowWrap:"normal",wordBreak:"normal"}}>{el.text||el.type}</span></div>}
