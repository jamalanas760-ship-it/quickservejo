import { Camera, Move, Utensils } from "lucide-react";
...
export function SmartCompositionCanvas({theme,composition,selectedId,onSelect,menuItems=[],restaurantName="Restaurant"}:Props){
 ...
 const hero=elements.find(e=>e.type==="image");
 const surfaceBg=composition?.background?.color??theme.bg;
 return <div className="smart-menu-canvas mx-auto w-full max-w-[960px] overflow-hidden rounded-[24px] shadow-2xl" style={{background:surfaceBg,color:theme.text}}>
  <div className="smart-menu-canvas-surface relative aspect-[4/3] w-full overflow-hidden" style={{fontFamily:fontMap[theme.bodyFont]??fontMap.sans}}>
   ...
   <div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_10%,rgba(255,255,255,.12),transparent_32%),linear-gradient(135deg,transparent_45%,rgba(0,0,0,.08))]" />
   {!elements.length&&<div className="absolute inset-0 grid place-items-center p-8 text-center"><div><Utensils className="mx-auto size-10 opacity-30"/><div className="mt-3 text-4xl font-black" style={{color:theme.primary}}>{restaurantName}</div><p className="mt-2 text-sm opacity-50">Generate a concept to activate the live menu.</p></div></div>}
   {elements.map(el=>{
    ...
    return <button key={el.id} type="button" aria-label={`Edit ${el.type}`} onClick={()=>onSelect(el.id)} className={`smart-menu-element absolute overflow-hidden border-0 bg-transparent p-0 text-left ${selected?"ring-2 ring-offset-1 ring-black/60":"hover:ring-1 hover:ring-black/20"}`} style={{left:`${left}%`,top:`${top}%`,width:`${width}%`,height:`${height}%`,zIndex:el.z??0,transform:`rotate(${el.rotation??0}deg)`,opacity:el.opacity??1,borderRadius:shape==="circle"?"999px":shape==="rounded"?`${Math.max(8,theme.radius)}px`:shape==="organic"?"34% 66% 48% 52% / 58% 42% 58% 42%":"10px",background:el.color??(isImage?`linear-gradient(135deg,${theme.primary}66,${theme.accent}55)`:"transparent"),animation:animation!=="none"?`${animation} 900ms ease both`:undefined}}>
      {isImage?<div className="relative h-full w-full overflow-hidden bg-black/10"><img src={el.image??""} alt="" className="h-full w-full object-cover" onError={(event)=>{event.currentTarget.style.display="none"}}/><div className="absolute inset-0 bg-gradient-to-tr from-black/20 via-transparent to-white/15"/>{!el.image&&<div className="absolute inset-0 grid place-items-center"><Camera className="size-7 opacity-20"/></div>}</div>
      ...
      :el.type==="shape"?<div className="h-full w-full" style={{background:el.color??theme.primary}}/>:<div className="flex h-full w-full min-w-0 items-center px-2 sm:px-3" style={{justifyContent:el.align==="center"?"center":el.align==="right"?"flex-end":"flex-start",textAlign:el.align??"left"}}><span className="block max-w-full overflow-hidden text-ellipsis" style={{fontSize:textSize?`clamp(10px,${Math.min(textSize,48)}px,${textSize}px)`:undefined,fontFamily:el.fontFamily||fontMap[theme.headingFont],color:el.color&&el.type!=="shape"?el.color:undefined,fontWeight:el.fontWeight,letterSpacing:el.letterSpacing!==undefined?`${clamp(el.letterSpacing,-2,12)}px`:undefined,lineHeight:el.lineHeight?clamp(el.lineHeight,.8,2):1.15,whiteSpace:"normal",overflowWrap:"normal",wordBreak:"normal"}}>{el.text||el.type}</span></div>}
      {selected&&<span className="absolute bottom-1 right-1 rounded-full bg-black/75 p-1 text-white"><Move className="size-3"/></span>}
    </button>;
   })}
   {hasRealMenu&&<div className="pointer-events-none absolute inset-x-[7%] bottom-[6%] z-[20] grid grid-cols-2 gap-2 sm:gap-3">
    {displayItems.map((item,index)=><div key={`live-${index}`} className="overflow-hidden rounded-xl border border-black/10 bg-white/90 p-1.5 shadow-sm backdrop-blur-sm">
      <div className="flex items-center gap-2"><div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-black/10">{item.image_url?<img src={item.image_url} alt="" className="h-full w-full object-cover"/>:<div className="grid h-full w-full place-items-center"><Camera className="size-3 opacity-20"/></div>}</div><div className="min-w-0 flex-1"><div className="truncate text-[9px] font-bold" style={{color:theme.text}}>{item.name_en||item.name_ar||"Menu item"}</div><div className="mt-0.5 truncate text-[7px]" style={{color:theme.muted}}>{item.description_en||item.description_ar||"Freshly prepared"}</div></div><div className="shrink-0 text-[8px] font-bold" style={{color:theme.primary}}>{item.price!=null?`${item.price} ${"JOD"}`:""}</div></div>
    </div>)}
   </div>}
   {hasRealMenu&&<div className="pointer-events-none absolute left-[7%] top-[6%] z-[21] max-w-[55%]"><div className="text-[8px] font-bold uppercase tracking-[.22em]" style={{color:theme.accent}}>{restaurantName}</div><div className="mt-1 truncate text-[clamp(18px,4vw,42px)] font-black leading-none" style={{fontFamily:fontMap[theme.headingFont],color:theme.text}}>{title}</div><div className="mt-1 text-[8px] font-bold uppercase tracking-[.18em]" style={{color:theme.muted}}>{category}</div></div>}
  </div>
 </div>;
}
