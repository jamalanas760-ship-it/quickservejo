  ...
  return <div className="space-y-5">
    ...
    <section className="qs-card min-w-0 overflow-hidden">
      ...
      <div className="grid gap-3 border-b border-border p-4 lg:grid-cols-[minmax(0,1fr)_180px_180px]">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">{ar ? "كل الحالات" : "All Status"}</SelectItem><SelectItem value="active">{t("common.active")}</SelectItem><SelectItem value="inactive">{t("common.inactive")}</SelectContent></Select>
      </div>
        ...
        {editing ? <>
          <div className="border-b border-border px-4 py-4 sm:px-6">
            <div className="flex min-w-0 items-center gap-3 pe-8">
              <span className="grid size-14 shrink-0 place-items-center overflow-hidden rounded-full bg-muted text-lg font-bold ring-1 ring-border">{(editing.avatar_url || avatarPresetUrl(editing.avatar_preset)) ? <img src={editing.avatar_url || avatarPresetUrl(editing.avatar_preset)} alt="" className="size-full object-cover" /> : editing.name.slice(0, 1).toUpperCase()}</span>
