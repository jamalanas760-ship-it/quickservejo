...
import { BarChart3, CalendarDays, FileText, ListPlus, MoreHorizontal, Palette, RotateCcw, Save, Settings2, SlidersHorizontal, Table2, Trash2, X } from "lucide-react";
...
export function AnalyticsManagerPro({ restaurantId, detailWidget: detailWidgetProp = null }: { restaurantId: string; detailWidget?: AnalyticsDetailWidgetId | null }) {
  ...
  function renderWidget(id: WidgetId) {
    ...
    if (id === "revenue") return <Widget title={labels[id]} tools={toolbar(id)}><div className="h-[150px]">{chart(id, data.series, "label", "sales")}</div></Widget>;
    if (id === "orders") return <Widget title={labels[id]} tools={toolbar(id)}><div className="mb-4 grid grid-cols-2 gap-2"><Kpi label={ar ? "30 يوم" : "30 days"} value={formatNumber(data.orders.length, lang)} /><Kpi label={ar ? "متوسط الطلب" : "Avg. order"} value={formatMoney(data.aov, currency, lang)} /></div><div className="h-[135px]">{chart(id, data.series, "label", "orders")}</div></Widget>;
    if (id === "channels") return <Widget title={labels[id]} tools={toolbar(id)}>{data.channels.length ? config.chartTypes[id] === "bar" ? <div className="h-[145px]">{chart(id, data.channels, "name", "value")}</div> : <div className="h-[185px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.channels} dataKey="value" nameKey="name" innerRadius={58} outerRadius={86} paddingAngle={4}>{data.channels.map((_, index) => <Cell key={index} fill={CHANNEL_COLORS[index % CHANNEL_COLORS.length]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer></div> : <Empty ar={ar} />}</Widget>;
    if (id === "topProducts") return <Widget title={labels[id]} tools={toolbar(id)}>{data.topProducts.length ? <div className="overflow-x-hidden"><table className="qs-table w-full table-fixed"><thead><tr><th>{ar ? "المنتج" : "Product"}</th><th>{ar ? "الكمية" : "Qty"}</th><th>{ar ? "الإيراد" : "Revenue"}</th></tr></thead><tbody>{data.topProducts.map((item) => { const row = { name: ar ? item.nameAr : item.name, qty: item.qty, revenue: item.revenue }; return <tr key={item.name} style={conditionalRowStyle(rules, row)}><td style={conditionalCellStyle(rules, "name", row.name)}>{row.name}</td><td style={conditionalCellStyle(rules, "qty", row.qty)}>{row.qty}</td><td style={conditionalCellStyle(rules, "revenue", row.revenue)}>{formatMoney(row.revenue, currency, lang)}</td></tr>; })}</tbody></table></div> : <Empty ar={ar} />}</Widget>;
    if (id === "peakHours") return <Widget title={labels[id]} tools={toolbar(id)}>{data.peak.length ? <div className="h-[190px]">{chart(id, data.peak.map((row) => ({ ...row, label: `${row.hour}:00` })), "label", "orders")}</div> : <Empty ar={ar} />}</Widget>;
    if (id === "weekly") return <Widget title={labels[id]} tools={toolbar(id)}><div className="h-[190px]">{chart(id, data.weekly, "label", "sales")}</div></Widget>;
  ...
  return <div className="qs-viewport-fill flex h-full min-h-0 flex-col gap-2">
    <MasterPageHeader
      ...
      description={ar ? "حوّل بيانات الطلبات الفعلية إلى اتجاهات واضحة، ساعات ذروة، منتجات رابحة وقنوات أداء." : "Turn real order data into revenue trends, peak hours, winning products and channel performance."}
    ...
    <div className="qs-scroll-region min-h-0 flex-1"><DashboardGrid ids={config.widgets} customize={customize} sizeFor={(id) => config.sizes[id] ?? DEFAULT_SIZES[id]} labelFor={(id) => labels[id]} minColumns={(id) => id === "orderTable" ? 8 : 4} minHeight={() => 150} onReorder={(source, target) => setConfig((current) => ({ ...current, widgets: reorderDashboardItems(current.widgets, source, target) }))} onResize={(id, size) => setConfig((current) => ({ ...current, sizes: { ...current.sizes, [id]: size } }))} renderItem={(id) => <div className="group relative h-full">{renderWidget(id)}{!customize ? <a href={`/manage/${restaurantId}/analytics/${id}`} className="absolute end-4 top-4 rounded-lg border border-border bg-card/95 px-2.5 py-1.5 text-[10px] font-bold text-[#e85d2a] opacity-0 shadow-sm transition group-hover:opacity-100 focus-visible:opacity-100">{ar ? "عرض التفاصيل" : "View details"}</a> : null}</div>} /></div>

    <Dialog open={addOpen} onOpenChange={(open) => { setAddOpen(open); if (!open) setWidgetSearch(""); }}><DialogContent className="max-h-[92vh] overflow-hidden p-0 sm:max-w-3xl"><DialogHeader className="border-b border-border p-5"><DialogTitle>{ar ? "مكتبة الأدوات" : "Widget Library"}</DialogTitle><DialogDescription>{ar ? "ابحث واختر أداة مبنية على بيانات المطعم الحالية." : "Search and add widgets powered by your current restaurant data."}</DialogDescription></DialogHeader><div className="p-5"><Input autoFocus value={widgetSearch} onChange={(event) => setWidgetSearch(event.target.value)} placeholder={ar ? "ابحث عن رسم أو تقرير أو جدول…" : "Search charts, reports or tables…"} className="mb-4"/><div className="max-h-[58vh] overflow-y-auto"><div className="grid gap-3 sm:grid-cols-2">{ALL_WIDGETS.filter((id) => labels[id].toLowerCase().includes(widgetSearch.toLowerCase())).map((id) => { const added=config.widgets.includes(id); const category=id === "orderTable" || id === "topProducts" ? (ar ? "جدول بيانات" : "Data table") : id === "summary" ? (ar ? "تقرير" : "Report") : id === "paidProgress" ? (ar ? "تقدم" : "Progress") : (ar ? "رسم بياني" : "Chart"); return <button key={id} type="button" disabled={added} className="group flex min-h-24 items-start gap-4 rounded-2xl border border-border p-4 text-start transition hover:border-orange-300 hover:shadow-sm disabled:cursor-not-allowed disabled:opacity-55" onClick={() => setConfig((current) => ({ ...current, widgets: [...current.widgets, id] }))}><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-orange-50 text-[#e85d2a] dark:bg-orange-950/30">{id === "orderTable" || id === "topProducts" ? <Table2 className="size-5" /> : id === "summary" ? <FileText className="size-5" /> : <BarChart3 className="size-5" />}</span><span className="min-w-0 flex-1"><strong className="block text-sm">{labels[id]}</strong><span className="mt-1 block text-[11px] text-muted-foreground">{category} · {ar ? "بيانات حقيقية لآخر 30 يوماً" : "Real data from the last 30 days"}</span><span className="mt-2 block text-[10px] font-bold text-[#e85d2a]">{added ? (ar ? "مضافة" : "Already added") : (ar ? "+ إضافة إلى اللوحة" : "+ Add to dashboard")}</span></span></button>; })}</div></div></div></DialogContent></Dialog>
...
function Kpi({ label, value, href, viewLabel }: { label: string; value: string; href?: string; viewLabel?: string }) { return <article className="qs-stat group flex min-h-[92px] flex-col p-3"><p className="text-[11px] font-semibold text-muted-foreground">{label}</p><p className="mt-1.5 truncate font-display text-xl font-bold tracking-[-.035em]">{value}</p>{href ? <a href={href} className="mt-auto pt-3 text-[10px] font-bold text-[#e85d2a] transition group-hover:translate-x-0.5">{viewLabel ?? "View details"} →</a> : null}</article>; }
