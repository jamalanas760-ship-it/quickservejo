...
export function AnalyticsManagerPro({ restaurantId, detailWidget: detailWidgetProp = null }: { restaurantId: string; detailWidget?: AnalyticsDetailWidgetId | null }) {
  ...
  return <div className="qs-viewport-fill flex h-full min-h-0 flex-col gap-2">
    <MasterPageHeader
      ...
      actions={<div className="flex flex-wrap gap-2"><span className="qs-button-secondary pointer-events-none"><CalendarDays className="size-4" />{ar ? "آخر 30 يوماً" : "Last 30 days"}</span>{customize ? <><Button variant="outline" onClick={() => { setConfig(savedConfig); setCustomize(false); }}><X className="size-4" />{ar ? "إلغاء" : "Cancel"}</Button><Button variant="outline" onClick={() => setConfig({ ...readConfig({ workspace: {} }), widgets: [...DEFAULT_WIDGETS] })}><RotateCcw className="size-4" />{ar ? "إعادة" : "Reset"}</Button><Button variant="outline" onClick={() => setAddOpen(true)}><ListPlus className="size-4" />{ar ? "إضافة أداة" : "Add Widget"}</Button><Button disabled={saving} onClick={() => void saveConfig()}><Save className="size-4" />{saving ? (ar ? "حفظ…" : "Saving…") : (ar ? "حفظ" : "Save Layout")}</Button></> : <button type="button" className="qs-button-secondary" onClick={() => setCustomize(true)}><Settings2 className="size-4" />{ar ? "تخصيص اللوحة" : "Customize Dashboard"}</button>}</div>}
    />
