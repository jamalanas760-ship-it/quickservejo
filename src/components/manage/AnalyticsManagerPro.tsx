...
export function AnalyticsManagerPro({ restaurantId, detailWidget: detailWidgetProp = null }: { restaurantId: string; detailWidget?: AnalyticsDetailWidgetId | null }) {
  ...
  function renderWidget(id: WidgetId) {
    ...
    if (id === "revenue") return <Widget title={labels[id]} tools={toolbar(id)}><div className="h-[190px]">{chart(id, data.series, "label", "sales")}</div></Widget>;
    if (id === "orders") return <Widget title={labels[id]} tools={toolbar(id)}><div className="mb-4 grid grid-cols-2 gap-2"><Kpi label={ar ? "30 يوم" : "30 days"} value={formatNumber(data.orders.length, lang)} /><Kpi label={ar ? "متوسط الطلب" : "Avg. order"} value={formatMoney(data.aov, currency, lang)} /></div><div className="h-[155px]">{chart(id, data.series, "label", "orders")}</div></Widget>;
    if (id === "channels") return <Widget title={labels[id]} tools={toolbar(id)}>{data.channels.length ? config.chartTypes[id] === "bar" ? <div className="h-[185px]">{chart(id, data.channels, "name", "value")}</div> : <div className="h-[185px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.channels} dataKey="value" nameKey="name" innerRadius={58} outerRadius={86} paddingAngle={4}>{data.channels.map((_, index) => <Cell key={index} fill={CHANNEL_COLORS[index % CHANNEL_COLORS.length]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer></div> : <Empty ar={ar} />}</Widget>;
