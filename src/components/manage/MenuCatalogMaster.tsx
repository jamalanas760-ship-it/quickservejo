...
export function MenuCatalogMaster({ restaurantId, mode = "products" }: { restaurantId: string; mode?: "categories" | "products" | "pricing" }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === "categories" ? <section className="qs-card overflow-hidden"><div className="flex items-center justify-between border-b border-border p-4"><div><h3 className="font-bold">{ar ? "فئات القائمة" : "Menu categories"}</h3><p className="text-xs text-muted-foreground">{ar ? "رتب وفعّل أقسام القائمة العادية." : "Organize and activate Standard Menu sections."}</p></div><Button onClick={() => setCategoryForm({ name_en: "", name_ar: "", is_active: true })}><Plus className="size-4" />{ar ? "إضافة فئة" : "Add Category"}</Button></div><div className="grid gap-3 p-4 sm:grid-cols-2">{categoryList.map((category) => <button key={category.id} type="button" onClick={() => setCategoryForm({ id: category.id, name_en: category.name_en, name_ar: category.name_ar, is_active: category.is_active })} className="flex min-h-20 items-center gap-3 rounded-xl border border-border p-4 text-start hover:bg-muted/30"><span className="grid size-10 place-items-center rounded-xl bg-orange-500/10 text-[#ff5a0a]"><UtensilsCrossed className="size-5" /></span><span className="min-w-0 flex-1"><strong className="block truncate">{pick(category.name_en, category.name_ar)}</strong><span className="text-xs text-muted-foreground">{standardProducts.filter(item => item.category_id === category.id).length} {ar ? "منتج" : "products"}</span></span><span className={cn("qs-status", category.is_active ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground")}>{category.is_active ? (ar ? "نشطة" : "Active") : (ar ? "متوقفة" : "Inactive")}</span><Pencil className="size-4 text-muted-foreground" /></button>)}</div></section> : null}

      {mode === "products" ? <div className="no-scrollbar flex gap-2 overflow-x-auto pb-1">
      ...
      {mode !== "categories" ? <section className="qs-card overflow-hidden">
        <div className="grid gap-3 border-b border-border p-4 md:grid-cols-[minmax(0,1fr)_180px_180px_auto]">
          ...
          {mode === "products" ? <button type="button" className="qs-button-primary min-h-11" onClick={() => setProductForm(emptyProduct(selectedCategory?.id ?? categoryList[0]?.id ?? ""))}><Plus className="size-4" />{ar ? "إضافة منتج" : "Add Product"}</button> : <span className="self-center text-xs font-bold text-muted-foreground">{ar ? "الأسعار والخيارات" : "Pricing & Options"}</span>}
        </div>
