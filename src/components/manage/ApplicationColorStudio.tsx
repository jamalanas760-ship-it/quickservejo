...
export function ApplicationColorStudio({ ar, restaurantName, brand, setBrand, primaryColor, accentColor, setPrimaryColor, setAccentColor }: Props) {
  ...
  return <div className="border-t border-border pt-6">
    <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
      <div><span className="inline-flex items-center gap-2 rounded-full bg-orange-500/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[.16em] text-[#ff5a0a]"><Palette className="size-3.5" />{ar ? "نظام الألوان" : "Color system"}</span><h4 className="mt-3 font-display text-xl font-bold tracking-[-.03em]">{ar ? "ألوان التطبيق" : "Application colors"}</h4><p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">{ar ? "الوضع الفاتح والداكن مستقلان تماماً. عدّل كل وضع بدون التأثير على الآخر." : "Light and dark mode are fully independent. Customize one without changing the other."}</p></div>
