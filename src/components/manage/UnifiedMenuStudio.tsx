...
import { BrainCircuit, Check, ExternalLink, FileImage, Layers3, Monitor, Minus, Plus, Save, Sparkles, Smartphone, Tablet, Wand2, X } from "lucide-react";
...
export function UnifiedMenuStudio({ restaurantId }: { restaurantId: string }) {
  ...
  const restaurant = useQuery({
    ...
    queryFn: async () => {
      const [{ data: restaurantData, error: restaurantError }, { data: menuData, error: menuError }] = await Promise.all([
        supabase.from("restaurants").select("id,name,slug,menu_theme,currency").eq("id", restaurantId).single(),
  ...
  function selectConcept(index: number) {
    ...
    try {
      ...
      setActive(index); setTheme(parseMenuTheme(raw)); setComposition(raw.composition); setSelectedNode(undefined); setMobileTab("canvas");
  ...
  function openLiveMenu() {
    ...
    if (!slug) { toast.error("Restaurant public URL is not available yet."); return; }
    const url = `${window.location.origin}/r/${encodeURIComponent(slug)}`;
    const existing = liveWindowRef.current;
    const target = existing && !existing.closed ? existing : window.open(url, "quickserve-live-menu", "popup,width=430,height=900,resizable=yes,scrollbars=yes");
    if (!target) { toast.error("Please allow pop-ups to open the live menu."); return; }
    liveWindowRef.current = target;
    target.focus();
    window.setTimeout(() => publishMenuThemeBridge(restaurantId, { ...theme, composition }, target), 300);
    toast.success("Live menu opened — changes now sync instantly");
  }
  ...
  const EditPanel = () => <section className="studio-mobile-panel rounded-2xl border bg-white p-4 shadow-sm">
    ...
    <p className="mb-3 text-xs leading-5 text-black/50">Select an element, describe the change, and only the necessary parts are redesigned.</p>
  ...
  const LayersPanel = () => <section className="studio-mobile-panel rounded-2xl border bg-white p-4 shadow-sm">
    ...
    {!composition?.elements?.length && <div className="rounded-xl bg-black/[.04] p-4 text-sm text-black/50">Generate a design first. AI-generated elements will appear here as editable layers.</div>}
  ...
  const PreviewPanel = () => (
    <main className="studio-preview-panel rounded-2xl border bg-[#dededb] p-2 shadow-sm sm:p-3 lg:p-4">
      <div className="studio-toolbar mb-2 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white/90 p-2 backdrop-blur">
        <div className="flex items-center gap-1 rounded-lg bg-black/[.05] p-1">
          {([ ["desktop", Monitor, "Desktop"], ["tablet", Tablet, "Tablet"], ["iphone", Smartphone, "iPhone"] ] as const).map(([key, Icon, label]) => <button key={key} type="button" onClick={() => setDevice(key)} className={cn("flex h-8 items-center gap-1.5 rounded-md px-2.5 text-[11px] font-bold", device === key ? "bg-black text-white shadow-sm" : "text-black/55 hover:bg-white")}><Icon className="size-3.5"/><span className="hidden sm:inline">{label}</span></button>)}
        </div>
        <div className="flex items-center gap-1 rounded-lg bg-black/[.05] p-1">
          <button type="button" aria-label="Zoom out" onClick={() => setZoom(v => Math.max(40, v - 8))} className="grid size-8 place-items-center rounded-md hover:bg-white"><Minus className="size-3.5"/></button>
          <span className="w-12 text-center text-[11px] font-bold text-black/55">{zoom}%</span>
          <button type="button" aria-label="Zoom in" onClick={() => setZoom(v => Math.min(120, v + 8))} className="grid size-8 place-items-center rounded-md hover:bg-white"><Plus className="size-3.5"/></button>
        </div>
      </div>
      ...
      <div className="studio-preview-stage flex min-h-[560px] items-start justify-center overflow-auto rounded-xl bg-[#c9c9c6] p-3 sm:p-6">
        {device === "iphone" ? (
          <div className="relative w-[390px] max-w-[92vw] rounded-[46px] border-[8px] border-black bg-black p-2 shadow-[0_30px_80px_rgba(0,0,0,.3)]" style={{ transform: `scale(${zoom / 72})`, transformOrigin: "top center" }}>
            <div className="pointer-events-none absolute left-1/2 top-2 z-[200] h-6 w-28 -translate-x-1/2 rounded-full bg-black" />
            <div className="overflow-hidden rounded-[36px] bg-white pt-1"><SmartCompositionCanvas theme={theme} composition={composition} selectedId={selectedNode} onSelect={handleCanvasSelect} menuItems={restaurant.data?.menuItems ?? []} restaurantName={restaurant.data?.name ?? "Restaurant"}/></div>
          </div>
        ) : device === "tablet" ? (
          <div className="w-[820px] max-w-[94vw] rounded-[30px] border-[8px] border-black bg-black p-2 shadow-[0_30px_80px_rgba(0,0,0,.22)]" style={{ transform: `scale(${zoom / 72})`, transformOrigin: "top center" }}><div className="overflow-hidden rounded-[22px] bg-white"><SmartCompositionCanvas theme={theme} composition={composition} selectedId={selectedNode} onSelect={handleCanvasSelect} menuItems={restaurant.data?.menuItems ?? []} restaurantName={restaurant.data?.name ?? "Restaurant"}/></div></div>
        ) : (
          <div className="w-full" style={{ transform: `scale(${zoom / 72})`, transformOrigin: "top center" }}><SmartCompositionCanvas theme={theme} composition={composition} selectedId={selectedNode} onSelect={handleCanvasSelect} menuItems={restaurant.data?.menuItems ?? []} restaurantName={restaurant.data?.name ?? "Restaurant"}/></div>
        )}
      </div>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2 rounded-xl bg-white/80 p-3 text-[11px] text-black/50"><span>{composition?.elements?.length ?? 0} editable layers · {device === "iphone" ? "iPhone live menu" : `${device} preview`}</span><Button size="sm" className="h-8 rounded-lg" onClick={openLiveMenu}><ExternalLink className="mr-1.5 size-3.5"/>Open live menu</Button></div>
