...
export function StaffManager({ restaurantId }: { restaurantId: string }) {
  ...
  }, [search, roleFilter, statusFilter, staff.data, tab]);
  ...
  return <div className="space-y-5">
    ...
    <div className={editing ? "grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]" : "grid gap-4"}>
      <section className="qs-card min-w-0 overflow-hidden">
        <div className="border-b border-border px-4 pt-3"><div className="flex gap-6 text-xs font-semibold">{([['all',ar?'كل الفريق':'All Staff',(staff.data??[]).length],['admins',ar?'المشرفون':'Admins',admins],['staff',ar?'الموظفون':'Staff',staffOnly]] as const).map(([id,label,count]) => <button key={id} type="button" onClick={() => setTab(id)} className={cn("relative min-h-11 px-1", tab===id ? "text-[#e85d2a]" : "text-muted-foreground")}><span>{label} ({count})</span>{tab===id ? <span className="absolute inset-x-0 bottom-0 h-0.5 bg-[#e85d2a]" />:null}</button>)}</div></div>
        <div className="grid gap-3 border-b border-border p-4 sm:grid-cols-[minmax(0,1fr)_170px_170px]">
          <div className="relative"><Search className="pointer-events-none absolute start-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder={ar?"ابحث بالاسم أو البريد...":"Search staff by name or email..."} className="h-11 ps-10" /></div>
          <Select value={roleFilter} onValueChange={(value)=>setRoleFilter(value as RoleFilter)}><SelectTrigger className="h-11"><SelectValue placeholder={ar?"كل الأدوار":"All Roles"} /></SelectTrigger><SelectContent><SelectItem value="all">{ar?"كل الأدوار":"All Roles"}</SelectItem>{ROLES.map((role)=><SelectItem key={role} value={role}>{STAFF_LABELS[role][lang]}</SelectItem>)}</SelectContent></Select>
          <Select value={statusFilter} onValueChange={(value)=>setStatusFilter(value as StatusFilter)}><SelectTrigger className="h-11"><SelectValue placeholder={ar?"كل الحالات":"All Status"} /></SelectTrigger><SelectContent><SelectItem value="all">{ar?"كل الحالات":"All Status"}</SelectItem><SelectItem value="active">{t('common.active')}</SelectItem><SelectItem value="inactive">{t('common.inactive')}</SelectItem></SelectContent></Select>
        </div>
...
function Permission({ label, checked }: { label:string; checked:boolean }) { return <label className="flex items-center gap-3"><input type="checkbox" checked={checked} readOnly className="size-4 accent-[#e85d2a]" /><span className="text-sm font-semibold">{label}</span></label>; }
