...
export function InventoryPanel({restaurantId,data,currency,onAction}:{restaurantId:string;data:BackOfficeData;currency:string;onAction:(request:RecordRequest)=>void}) {
  ...
  return <section className="space-y-4">
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card">
      ...
      {!rows.length?<div className="grid min-h-[260px] place-items-center p-8 text-center"><div><Package className="mx-auto size-9 text-muted-foreground"/><h4 className="mt-3 font-bold">{ar?"لا توجد مواد مطابقة":"No matching inventory"}</h4><p className="mt-1 text-xs text-muted-foreground">{ar?"أضف مادة أو غيّر الفلاتر.":"Add an item or adjust the filters."}</p></div></div>:<>
        <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[900px] text-sm"><thead className="bg-muted/45 text-xs text-muted-foreground"><tr><th className="px-4 py-3 text-start">{ar?"المادة":"Item"}</th><th className="px-4 py-3 text-start">{ar?"الحالة":"Status"}</th><th className="px-4 py-3 text-end">{ar?"المتاح":"On hand"}</th><th className="px-4 py-3 text-end">{ar?"إعادة الطلب":"Reorder"}</th><th className="px-4 py-3 text-end">{ar?"تكلفة الوحدة":"Unit cost"}</th><th className="px-4 py-3 text-end">{ar?"قيمة المخزون":"Stock value"}</th><th className="px-4 py-3 text-end">{ar?"إجراءات":"Actions"}</th></tr></thead><tbody className="divide-y divide-border">{rows.map(item=>{const cost=latestCost.get(item.id)??0;const value=Number(item.quantity)*cost;return <tr key={item.id} className="transition hover:bg-muted/20"><td className="px-4 py-4"><strong>{item.name}</strong><span className="ms-2 text-xs text-muted-foreground">{item.unit}</span></td><td className="px-4 py-4"><StockBadge state={stateOf(item)} ar={ar}/></td><td className="px-4 py-4 text-end font-semibold tabular-nums">{formatNumber(Number(item.quantity),lang)} {item.unit}</td><td className="px-4 py-4 text-end text-muted-foreground tabular-nums">{formatNumber(Number(item.reorder_level),lang)}</td><td className="px-4 py-4 text-end tabular-nums">{cost?formatMoney(cost,currency,lang):"—"}</td><td className="px-4 py-4 text-end font-semibold tabular-nums">{cost?formatMoney(value,currency,lang):"—"}</td><td className="px-4 py-4"><div className="flex justify-end gap-1.5"><Button size="sm" onClick={()=>onAction({kind:"receive",itemId:item.id})}>{ar?"استلام":"Receive"}</Button><Button size="sm" variant="outline" disabled={Number(item.quantity)<=0} onClick={()=>onAction({kind:"issue",itemId:item.id})}>{ar?"صرف":"Issue"}</Button><Button size="icon" variant="ghost" onClick={()=>setHistoryItem(item)} aria-label={ar?"السجل":"History"}><History className="size-4"/></Button><Button size="icon" variant="ghost" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={()=>setDeleteItem(item)} aria-label={ar?"حذف المادة":"Delete item"}><Trash2 className="size-4"/></Button></div></td></tr>})}</tbody></table></div>
        <div className="space-y-2 p-3 md:hidden">{rows.map(item=>{const cost=latestCost.get(item.id)??0;return <article key={item.id} className="rounded-xl border border-border p-3"><div className="flex items-start justify-between gap-3"><div><strong className="block">{item.name}</strong><p className="mt-1 text-xs text-muted-foreground">{formatNumber(Number(item.quantity),lang)} {item.unit} · {ar?"حد":"reorder"} {formatNumber(Number(item.reorder_level),lang)}</p></div><StockBadge state={stateOf(item)} ar={ar}/></div><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg bg-muted/45 p-2"><span className="text-muted-foreground">{ar?"تكلفة الوحدة":"Unit cost"}</span><strong className="mt-1 block">{cost?formatMoney(cost,currency,lang):"—"}</strong></div><div className="rounded-lg bg-muted/45 p-2"><span className="text-muted-foreground">{ar?"القيمة":"Value"}</span><strong className="mt-1 block">{cost?formatMoney(cost*Number(item.quantity),currency,lang):"—"}</strong></div></div><div className="mt-3 grid grid-cols-4 gap-2"><Button size="sm" onClick={()=>onAction({kind:"receive",itemId:item.id})}>{ar?"استلام":"Receive"}</Button><Button size="sm" variant="outline" disabled={Number(item.quantity)<=0} onClick={()=>onAction({kind:"issue",itemId:item.id})}>{ar?"صرف":"Issue"}</Button><Button size="sm" variant="ghost" onClick={()=>setHistoryItem(item)} aria-label={ar?"السجل":"History"}><History className="size-4"/></Button><Button size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={()=>setDeleteItem(item)} aria-label={ar?"حذف المادة":"Delete item"}><Trash2 className="size-4"/></Button></div></article>})}</div>
      </>}
    </section>
    ...
    <Dialog open={Boolean(deleteItem)} onOpenChange={open=>{if(!open)setDeleteItem(null)}}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>{ar?"سيتم إخفاء المادة من المخزون مع الاحتفاظ بالحركات والسجل المالي المرتبط بها لأغراض المراجعة.":"The item will be removed from active inventory while its movement and Finance history stays preserved for audit."}</DialogDescription>
        </DialogHeader>
        {deleteItem?<div className="rounded-2xl border border-border bg-muted/35 p-4"><strong className="block">{deleteItem.name}</strong><p className="mt-1 text-xs text-muted-foreground">{formatNumber(Number(deleteItem.quantity),lang)} {deleteItem.unit} · {ar?"حد إعادة الطلب":"reorder"} {formatNumber(Number(deleteItem.reorder_level),lang)}</p></div>:null}
        <DialogFooter>
          <Button variant="outline" onClick={()=>setDeleteItem(null)}>{ar?"إلغاء":"Cancel"}</Button>
          <Button variant="destructive" disabled={!deleteItem||write.isPending} onClick={()=>{if(!deleteItem)return;const item=deleteItem;write.mutate({kind:"delete_item",item_id:item.id,name:item.name},{onSuccess:()=>{toast.success(ar?"تم حذف المادة من المخزون":"Inventory item deleted");setDeleteItem(null)},onError:(error)=>toast.error(humanError(error,lang))})}}><Trash2 className="size-4"/>{ar?"حذف المادة":"Delete item"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
