  ...
  return <section className="space-y-5">
    <div className="grid gap-4 xl:grid-cols-2">
      <article className="qs-card p-5">
        <div className="flex items-start gap-3"><span className="grid size-10 place-items-center rounded-xl bg-orange-500/10 text-[#e85d2a]"><KeyRound className="size-4"/></span><div><h2 className="font-display text-lg font-bold">{ar?"مفاتيح API":"API keys"}</h2><p className="mt-1 text-xs text-muted-foreground">{ar?"مفاتيح محددة الصلاحيات للتكاملات الخارجية. يتم تخزين البصمة فقط.":"Scoped keys for external systems. Only the key hash is stored."}</p></div></div>
      ...
      <article className="qs-card p-5">
        <div className="flex items-start gap-3"><span className="grid size-10 place-items-center rounded-xl bg-orange-500/10 text-[#e85d2a]"><Webhook className="size-4"/></span><div><h2 className="font-display text-lg font-bold">{ar?"Webhooks موقعة":"Signed webhooks"}</h2><p className="mt-1 text-xs text-muted-foreground">{ar?"إرسال تلقائي مع HMAC، إعادة المحاولة وسجل التسليم.":"Automatic HMAC-signed delivery with retries and delivery logs."}</p></div></div>
        <div className="mt-4 space-y-3">
    ...
    <article className="qs-card p-5">
      <div className="flex items-center gap-3"><ShieldCheck className="size-5 text-[#e85d2a]"/><div><h2 className="font-bold">{ar?"سجل تسليم Webhook":"Webhook delivery log"}</h2><p className="text-xs text-muted-foreground">{ar?"آخر 30 محاولة؛ تتم إعادة المحاولة تلقائياً حتى 5 مرات.":"Latest 30 attempts; failed deliveries retry automatically up to five times."}</p></div></div>
...
function SecretReveal({title,value,onCopy,onClose}:{title:string;value:string;onCopy:(value:string)=>void|Promise<void>;onClose:()=>void}) {
  return <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50/60 p-3 dark:border-amber-900/50 dark:bg-amber-950/10"><div className="flex items-center justify-between gap-3"><strong className="text-xs">{title}</strong><button type="button" onClick={onClose} aria-label="Close"><X className="size-4"/></button></div><div className="mt-2 flex gap-2"><code className="min-w-0 flex-1 break-all rounded-lg bg-background p-2 text-[11px]">{value}</code><Button size="sm" variant="outline" onClick={()=>void onCopy(value)}><Copy className="size-3"/></Button></div></div>;
}
