...
function RoleAvatarFallback({ role, superAdmin }: { role: string | null | undefined; superAdmin: boolean }) {
  ...
  if (role === "manager" || role === "operations_manager") return <UserRoundCog className={iconClass} />;
...
function RestaurantLogo({ src, name, compact = false }: { src: string; name: string; compact?: boolean }) {
  return (
    <span className={cn("inline-flex shrink-0 items-center justify-center overflow-hidden rounded-xl border border-border/70 bg-white px-2 shadow-sm", compact ? "h-10 max-w-[132px]" : "h-11 max-w-[168px]")}>
      <img src={src} alt={name} className="h-8 w-auto max-w-full object-contain" />
    </span>
  );
}
...
export function AppHeader({ onMenu, className, title }: { onMenu?: () => void; className?: string; title?: string | undefined }) {
  ...
  const actionCount = counters.data?.total ?? 0;
  const homeTo = membership?.role === "manager" || membership?.role === "operations_manager" ? "/manager" : access.isSuperAdmin ? "/super-admin" : "/dashboard";
  const actionTitle = lang === "ar"
    ? `${counters.data?.tasks ?? 0} مهام · ${counters.data?.shifts ?? 0} ورديات · ${counters.data?.orders ?? 0} طلبات`
    : `${counters.data?.tasks ?? 0} tasks · ${counters.data?.shifts ?? 0} shifts · ${counters.data?.orders ?? 0} orders`;
    ...
    <header className={cn("qs-topbar safe-top sticky top-0 z-40", className)}>
      <div className="mx-auto flex h-[68px] w-full max-w-[1600px] items-center gap-3 px-3 sm:px-5 lg:px-6">
        ...
        <Link to={homeTo as never} className="min-w-0 shrink-0 lg:hidden" aria-label={restaurant?.name || "QuickServe"}>
          {customLogo ? <RestaurantLogo src={restaurant!.logo_url!} name={restaurant?.name ?? "Restaurant"} compact /> : <BrandLogo className="size-8" accentClassName="text-[#ff5a0a]" textClassName="text-lg text-foreground" />}
        </Link>

        <div className="hidden min-w-0 items-center gap-3 lg:flex">
          {selectedId ? <RestaurantSwitcher restaurantId={selectedId} /> : (
            <div className="flex min-w-0 items-center gap-3">
              {customLogo ? <RestaurantLogo src={restaurant!.logo_url!} name={restaurant?.name ?? "Restaurant"} /> : null}
        ...
        <div className="ms-auto flex items-center gap-1 sm:gap-1.5">
          ...
          <Link to="/notifications" title={actionTitle} className="relative grid size-10 place-items-center rounded-xl text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label={lang === "ar" ? `الإشعارات، ${actionCount} عناصر تحتاج انتباهك` : `Notifications, ${actionCount} items need attention`}>
