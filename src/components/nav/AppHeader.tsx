...
export function AppHeader({ onMenu, className, title }: { onMenu?: () => void; className?: string; title?: string | undefined }) {
  ...
  const [initial, setInitial] = useState("Q");
  ...
  return (
    <header className={cn("qs-topbar safe-top sticky top-0 z-40", className)}>
      <div className="mx-auto flex h-[70px] w-full max-w-[1600px] items-center gap-3 px-3 sm:px-5 lg:px-7">
        {onMenu ? (
          <button type="button" onClick={onMenu} aria-label="Menu" className="grid size-10 shrink-0 place-items-center rounded-xl border bg-card lg:hidden">
            <MenuIcon className="size-5" />
          </button>
        ) : null}

        <Link to={access.isSuperAdmin ? "/super-admin" : "/dashboard"} className="shrink-0 lg:hidden">
          <BrandLogo className="size-8" accentClassName="text-[#ff5a0a]" textClassName="hidden text-lg sm:inline" />
        </Link>
        ...
        <div className="relative hidden min-w-0 flex-1 lg:block lg:max-w-[480px]">
          ...
          <input
            ...
            className="qs-topbar-search w-full ps-11 pe-4 text-[13px] outline-none placeholder:text-muted-foreground focus:border-[#ff5a0a] focus:ring-2 focus:ring-[#ff5a0a]/10"
            placeholder={lang === "ar" ? "ابحث في الطلبات والقائمة والموظفين..." : "Search orders, menu items, staff..."}
          />
        </div>
        ...
        <div className="ms-auto flex items-center gap-1.5 sm:gap-2">
          <button type="button" className="qs-control hidden h-10 items-center gap-2 px-3 text-[12px] font-semibold md:flex">
            <MapPin className="size-4 text-muted-foreground" />
            <span className="max-w-40 truncate">{restaurant?.name || scope.restaurantName || (access.isSuperAdmin ? "All Locations" : "Restaurant")}</span>
            <ChevronDown className="size-3.5 text-muted-foreground" />
          </button>

          <button type="button" className="qs-control hidden h-10 items-center gap-2 px-3 text-[12px] font-semibold xl:flex">
            <CalendarDays className="size-4 text-muted-foreground" />
            <span>{lang === "ar" ? "اليوم، " : "Today, "}{dateLabel}</span>
            <ChevronDown className="size-3.5 text-muted-foreground" />
          </button>

          <ThemeToggle compact />

          <button type="button" onClick={toggleLang} className="grid size-10 place-items-center rounded-xl text-muted-foreground transition hover:bg-muted hover:text-foreground" aria-label={lang === "ar" ? "Switch to English" : "التبديل إلى العربية"}>
          ...
          <Link to="/profile" aria-label="Profile" className="grid size-10 place-items-center rounded-full bg-muted font-display text-sm font-bold text-foreground ring-1 ring-border">
            {initial}
          </Link>
        </div>
      </div>
