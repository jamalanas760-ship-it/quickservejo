...
const SCOPES = {
  figma: "file_content:read file_metadata:read",
  canva: "design:content:read design:content:write design:meta:read asset:read asset:write brandtemplate:meta:read brandtemplate:content:read",
  adobe: "openid",
} as const;
...
function redirectUri(provider: CreativeProvider) {
...
function authUrl(provider: CreativeProvider, state: string, codeChallenge?: string) {
  ...
  if (provider === "canva" || provider === "figma") {
    if (!codeChallenge) throw new Error("PKCE challenge is required.");
    params.set("code_challenge", codeChallenge);
    params.set("code_challenge_method", "S256");
  }
  const endpoint = provider === "canva"
...
export const beginCreativeIntegration = createServerFn({ method: "POST" })
...
async function exchangeCanva(code: string, verifier: string) {
  ...
  if (!response.ok) throw new Error(`Canva OAuth failed (${response.status}).`);
...
async function exchangeFigma(code: string, verifier?: string) {
  ...
  if (!response.ok) throw new Error(`Figma OAuth failed (${response.status}).`);
  return response.json();
}
...
async function exchangeAdobe(code: string) {
  ...
  if (!response.ok) throw new Error(`Adobe OAuth failed (${response.status}).`);
  return response.json();
}

export const exchangeCreativeIntegration = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => tokenInput.parse(input))
  .handler(async ({ data }) => {
    if (data.provider === "canva" && !data.codeVerifier) throw new Error("Canva requires the PKCE verifier.");
    const token = data.provider === "canva"
      ? await exchangeCanva(data.code, data.codeVerifier!)
      : data.provider === "figma"
        ? await exchangeFigma(data.code, data.codeVerifier)
        : await exchangeAdobe(data.code);
    return { provider: data.provider, expiresIn: token.expires_in ?? null, connected: true };
  });

export const canvaCreateAutofill = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ accessToken: z.string().min(1), templateId: z.string().min(1), data: z.record(z.string(), z.unknown()) }).parse(input))
  .handler(async ({ data }) => {
    const response = await fetch("https://api.canva.com/rest/v1/autofills", {
      ...
      body: JSON.stringify({ type: "create_from_brand_template", brand_template_id: data.templateId, data: data.data }),
    });
    if (!response.ok) throw new Error(`Canva autofill failed (${response.status}).`);
    return response.json();
  });

export const figmaFileMetadata = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  ...
  .handler(async ({ data }) => {
    ...
    if (!response.ok) throw new Error(`Figma file lookup failed (${response.status}).`);
    return response.json();
  });

export const adobeExpressConfig = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async () => ({ clientId: env("ADOBE_CLIENT_ID"), appName: env("ADOBE_EXPRESS_APP_NAME") }));
