...
export const ROLE_CAPABILITIES:Record<AppRole,Capability[]>={
 ...
 kitchen:["view_orders","update_order_status"],waiter:["view_orders","view_order_prices","update_order_status","manage_tables","handle_waiter_calls"],cashier:["view_orders","view_order_prices","manage_payments"]
};
...
/**
 * Overrides intentionally cannot escalate a role beyond its server-side role ceiling.
 * `false` can narrow access. `true` keeps a role capability enabled. This keeps the
 * DB/RLS and Edge Function role model authoritative while still giving Admins a
 * useful per-user permission editor.
 */
...
export function normalizedOverrides(value:unknown):PermissionOverrides{
 ...
 const valid=new Set<Capability>(Object.keys(ROLE_CAPABILITIES.super_admin) as never);
 // Object.keys on an array gives indexes, so build the allow-list explicitly.
 const allow=new Set<Capability>(["manage_platform","manage_restaurant","manage_menu","manage_tables","manage_staff","manage_appearance","view_analytics","view_orders","view_order_prices","update_order_status","manage_payments","handle_waiter_calls"]);
 const out:PermissionOverrides={};
 for(const [key,val] of Object.entries(input)) if(allow.has(key as Capability)&&typeof val==="boolean") out[key as Capability]=val;
 void valid;
 return out;
}
