 ...
 *
 * The `erp_*` tables were added by a migration that post-dates the last
 * Supabase type generation, so this module owns the single boundary cast and
 * exposes only narrowly typed helpers. Components never cast the client.
 */
...
function erpFrom(table: ErpTableName): ErpBuilder {
  return (supabase as unknown as { from(table: string): ErpBuilder }).from(table);
}

/** Runs a read against an ERP table and returns typed rows. */
export async function erpSelect<T>(
  table: ErpTableName,
  build: (query: ErpBuilder) => ErpBuilder,
): Promise<T[]> {
...
/** Appends a tenant-scoped ERP record. RLS and the stock trigger stay authoritative. */
export async function erpInsert(
  table: Exclude<ErpTableName, "erp_inventory_balances">,
  restaurantId: string,
  payload: Record<string, unknown>,
): Promise<void> {
  const { error } = await erpFrom(table)
    .insert({ ...payload, restaurant_id: restaurantId })
    .select("id");
  if (error) throw error;
}

export function isLowStock(item: InventoryBalance): boolean {
  return Number(item.quantity) <= Number(item.reorder_level);
}

/**
 * Approximate stock value using the most recent non-zero unit cost recorded for
 * each item. Items that never had a cost recorded are reported separately so the
 * number is never silently understated.
 */
export function stockValuation(
  items: InventoryBalance[],
  movements: StockMovement[],
): { value: number; pricedItems: number; itemsWithoutCost: number } {
  ...
  for (const item of items) {
    ...
    if (cost === undefined) {
      if (Number(item.quantity) > 0) itemsWithoutCost += 1;
      continue;
    }
...
/** Client-side CSV download from already-filtered rows. */
