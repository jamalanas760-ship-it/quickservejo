...
function callback(provider: CreativeProvider) {
  return `${appUrl()}/integrations/${provider}/callback`;
}
...
export const startCreativeOAuth = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ provider, codeChallenge: z.string().min(43).max(128).optional() }).parse(input))
  .handler(async ({ data }) => {
    if (data.provider === "adobe") {
      return { mode: "embed" as const, provider: data.provider, clientId: required("ADOBE_EXPRESS_CLIENT_ID"), appName: required("ADOBE_EXPRESS_APP_NAME") };
    }
    ...
    const scope = data.provider === "canva"
      ? "design:content:read design:content:write design:meta:read asset:read asset:write brandtemplate:meta:read brandtemplate:content:read"
    ...
    const authorizationUrl = data.provider === "canva"
...
async function exchange(provider: "figma" | "canva", code: string, verifier: string) {
  const clientId = required(`${provider.toUpperCase()}_CLIENT_ID`);
  const clientSecret = required(`${provider.toUpperCase()}_CLIENT_SECRET`);
  ...
  if (!response.ok) throw new Error(`${provider} OAuth exchange failed (${response.status}).`);
  return response.json() as Promise<{ access_token: string; refresh_token?: string; expires_in?: number; user_id_string?: string }>;
}

export const finishCreativeOAuth = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ provider: z.enum(["figma", "canva"]), code: z.string().min(1), codeVerifier: z.string().min(43).max(128), state: z.string().min(16), expectedState: z.string().min(16) }).parse(input))
  .handler(async ({ data }) => {
...
export const canvaAutofill = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ accessToken: z.string().min(1), brandTemplateId: z.string().min(1), data: z.record(z.string(), z.unknown()) }).parse(input))
  .handler(async ({ data }) => {
    ...
    if (!response.ok) throw new Error(`Canva autofill failed (${response.status}).`);
    return response.json();
  });

export const figmaFile = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ accessToken: z.string().min(1), fileKey: z.string().min(22) }).parse(input))
  .handler(async ({ data }) => {
    ...
    if (!response.ok) throw new Error(`Figma file read failed (${response.status}).`);
    return response.json();
  });
