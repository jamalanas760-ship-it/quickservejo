...
function PublicBookingPage(){
  ...
  if(created)return <main className="min-h-dvh bg-muted/20 p-4 sm:p-8"><section className="mx-auto max-w-xl overflow-hidden rounded-[32px] border bg-card shadow-xl"><div className="p-6 text-center sm:p-9"><span className="mx-auto grid size-16 place-items-center rounded-full bg-emerald-500/10 text-emerald-600"><CheckCircle2 className="size-8"/></span><h1 className="mt-5 font-display text-3xl font-bold">{ar?"تم استلام الحجز":"Reservation received"}</h1><p className="mt-2 text-sm text-muted-foreground">{created.status==="confirmed"?(ar?"الحجز مؤكد.":"Your reservation is confirmed."):(ar?"الحجز قيد التأكيد من المطعم.":"The restaurant will confirm your reservation shortly.")}</p><div className="mt-6 rounded-2xl bg-muted/45 p-5"><p className="text-xs text-muted-foreground">{ar?"رمز الحجز":"Confirmation code"}</p><strong className="mt-1 block font-mono text-3xl tracking-[.16em]">{created.confirmation_code}</strong><div className="mt-4 grid grid-cols-2 gap-3 text-start text-sm"><Info label={ar?"الموعد":"Date & time"} value={new Intl.DateTimeFormat(ar?"ar-JO":"en-US",{dateStyle:"medium",timeStyle:"short",timeZone:restaurant.timezone}).format(new Date(created.booking_at))}/><Info label={ar?"عدد الضيوف":"Guests"} value={String(created.guest_count)}/></div>{created.deposit_amount>0?<div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-start text-xs text-amber-900 dark:border-amber-900/50 dark:bg-amber-950/20 dark:text-amber-200">{ar?"يتطلب الحجز عربوناً بقيمة ":"A reservation deposit is required: "}{formatMoney(created.deposit_amount,created.currency,lang)}. {ar?"سيظهر الدفع عند تفعيل مزود الدفع للمطعم.":"Payment becomes available when the restaurant payment provider is configured."}</div>:null}</div><Button asChild className="mt-6 w-full"><Link to="/booking/$token" params={{token:created.public_token}}>{ar?"إدارة الحجز":"Manage booking"}</Link></Button><Button asChild variant="ghost" className="mt-2 w-full"><Link to="/r/$slug" params={{slug}}>{ar?"عرض القائمة":"View menu"}</Link></Button></div></section></main>;
    ...
    <section className="mx-auto max-w-3xl overflow-hidden rounded-[32px] border bg-card shadow-xl">
      ...
      <div className="space-y-6 p-5 sm:p-8">
        ...
        <section>
          ...
          {slots.isPending?<Skeleton className="mt-3 h-28 rounded-2xl"/>:slots.isError?<p className="mt-3 text-sm text-destructive">{humanError(slots.error,lang)}</p>:(slots.data??[]).length===0?<div className="mt-3 rounded-2xl border border-dashed p-6 text-center text-sm text-muted-foreground">{ar?"لا توجد طاولة متاحة لهذا اليوم والحجم. جرّب تاريخاً أو عدداً مختلفاً.":"No table is available for this date and party size. Try another date or party size."}</div>:<div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-5">{(slots.data??[]).map(row=><button key={row.slot_at} type="button" onClick={()=>setSlot(row.slot_at)} className={cn("rounded-xl border px-3 py-3 text-sm font-bold transition",slot===row.slot_at?"border-[#ff5a0a] bg-orange-500/10 text-[#ff5a0a]":"border-border hover:bg-muted/40")}><span className="block">{new Intl.DateTimeFormat(ar?"ar-JO":"en-US",{hour:"2-digit",minute:"2-digit",timeZone:restaurant.timezone}).format(new Date(row.slot_at))}</span><span className="mt-1 block text-[9px] font-medium text-muted-foreground">{row.available_tables} {ar?"طاولات":"tables"}</span></button>)}</div>}
        </section>
