...
function HostWorkspace() {
  ...
  return <div className="min-h-dvh bg-background">
    ...
    <main className="qs-page space-y-5">
      ...
      : <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{rows.map((table) => <article key={table.id} className={cn("qs-card p-4 transition", table.pendingCall && "ring-2 ring-orange-400/70")}><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[.12em] text-muted-foreground">{table.zone || (ar ? "المنطقة الرئيسية" : "Main area")}</p><h2 className="mt-1 font-display text-xl font-bold">{table.table_name || `${ar ? "طاولة" : "Table"} ${table.table_number}`}</h2><p className="mt-1 text-xs text-muted-foreground">#{table.table_number}</p></div><Badge variant={table.busy ? "secondary" : "outline"}>{table.busy ? (ar ? "مشغولة" : "Busy") : (ar ? "متاحة" : "Available")}</Badge></div>{table.pendingCall ? <div className="mt-4 flex items-center gap-2 rounded-xl bg-orange-500/10 px-3 py-2 text-xs font-bold text-orange-700 dark:text-orange-300"><BellRing className="size-4" />{ar ? "الضيف طلب المساعدة" : "Guest requested assistance"}</div> : null}</article>)}</section>}
    </main>
  </div>;
}
