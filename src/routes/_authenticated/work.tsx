      ...
      <section className="qs-card qs-viewport-fill flex min-h-0 flex-col overflow-hidden">
        <div className="border-b border-border p-3 sm:p-4">
          <div className="grid gap-2 xl:grid-cols-[auto_minmax(0,1fr)] xl:items-center">
            ...
            {tab !== "handover" ? <div className="flex flex-wrap items-center gap-2 xl:justify-end"><div className="qs-search-field min-w-[210px] flex-1 xl:max-w-[300px]"><Search /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="qs-search-input" placeholder={ar ? "بحث بالعنوان أو الوصف" : "Search work"} /></div><Button type="button" variant="outline" onClick={()=>setFiltersOpen(value=>!value)} aria-expanded={filtersOpen}><SlidersHorizontal className="size-4"/>{ar?"الفلاتر":"Filters"}</Button><div className="inline-grid grid-cols-2 rounded-[10px] border border-border p-0.5"><button type="button" onClick={() => setViewMode("cards")} aria-label={ar ? "عرض بطاقات" : "Card view"} className={cn("grid size-10 place-items-center rounded-lg", viewMode === "cards" ? "bg-muted text-foreground" : "text-muted-foreground")}><LayoutGrid className="size-4" /></button><button type="button" onClick={() => setViewMode("list")} aria-label={ar ? "عرض قائمة" : "List view"} className={cn("grid size-10 place-items-center rounded-lg", viewMode === "list" ? "bg-muted text-foreground" : "text-muted-foreground")}><Rows3 className="size-4" /></button></div></div> : null}
          </div>
