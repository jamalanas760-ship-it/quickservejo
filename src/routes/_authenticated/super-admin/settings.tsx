      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(340px,.8fr)]">
        <form className="space-y-4" onSubmit={(event) => { event.preventDefault(); void save(); }}>
          <section className="qs-card p-5 sm:p-6">
            ...
            <div className="mt-6 flex justify-end"><Button type="submit" disabled={saving} className="bg-[#e85d2a] text-white hover:bg-[#e94f00]"><Save className="size-4" />{saving ? (ar ? "جارٍ الحفظ…" : "Saving…") : (ar ? "حفظ التغييرات" : "Save Changes")}</Button></div>
          </section>
        ...
        <aside className="space-y-4">
          ...
          <section className="qs-card p-5"><div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-full bg-orange-500/12 text-[#e85d2a]"><Bell className="size-5" /></span><div><h2 className="font-bold">{ar ? "تنبيهات التشغيل" : "Operational Alerts"}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">{ar ? "حالة المطاعم والتنبيهات تظهر من الشريط العلوي." : "Restaurant health and platform alerts are available from the top bar."}</p></div></div></section>
        </aside>
      </div>
    </div>
  );
