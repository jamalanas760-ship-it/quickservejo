...
function DevicesPage(){
  ...
  return <div className="min-h-dvh bg-background">
    ...
    <main className="qs-page qs-compact-page qs-viewport-page">
      ...
      <section className="qs-viewport-fill grid min-h-0 gap-2 overflow-hidden xl:grid-cols-[250px_minmax(0,1fr)_300px]">
        <article className="qs-card qs-scroll-region min-h-0 p-3">
          <div className="flex items-start gap-2"><span className="grid size-8 place-items-center rounded-[9px] bg-orange-500/10 text-[#ff5a0a]"><TabletSmartphone className="size-3.5"/></span><div><h2 className="text-sm font-bold">{ar?"تسجيل هذا الجهاز":"Register this device"}</h2><p className="mt-0.5 text-[10px] leading-4 text-muted-foreground">{ar?"سجل هذا المتصفح كجهاز QuickServe وراقب توفره.":"Register this browser as a QuickServe device and monitor its availability."}</p></div></div>
          <div className="mt-3 space-y-2">
            ...
            <Button className="w-full" disabled={register.isPending||name.trim().length<1} onClick={()=>register.mutate()}><PlugZap className="size-3.5"/>{ar?"تسجيل ومراقبة":"Register & monitor"}</Button>
          </div>
          {newToken?<div className="mt-3 rounded-[10px] border border-amber-200 bg-amber-50/60 p-2.5 dark:border-amber-900/50 dark:bg-amber-950/10"><strong className="text-[10px]">{ar?"رمز الجهاز يظهر مرة واحدة":"Device token — shown once"}</strong><div className="mt-2 flex gap-2"><code className="min-w-0 flex-1 break-all rounded-lg bg-background p-2 text-[9px]">{newToken}</code><Button size="sm" variant="outline" onClick={()=>void copyToken()}><Copy className="size-3"/></Button></div></div>:null}
        </article>

        <article className="qs-card flex min-h-0 flex-col overflow-hidden">
          <div className="flex items-center justify-between gap-3 border-b border-border p-3"><div><h2 className="qs-section-title">{ar?"الأجهزة المسجلة":"Registered devices"}</h2><p className="mt-0.5 text-[10px] text-muted-foreground">{ar?"Online = نبضة صحة حديثة.":"Online means a recent successful heartbeat."}</p></div><Button variant="outline" size="sm" onClick={()=>qc.invalidateQueries({queryKey:["restaurant-devices",rid]})}><RefreshCw className="size-3"/>{ar?"تحديث":"Refresh"}</Button></div>
          {devices.isPending?<div className="p-4"><Skeleton className="h-60 rounded-xl"/></div>:rows.length===0?<div className="grid min-h-0 flex-1 place-items-center p-4 text-center"><div><MonitorSmartphone className="mx-auto size-8 text-muted-foreground"/><p className="mt-2 text-xs text-muted-foreground">{ar?"لم يتم تسجيل أجهزة بعد.":"No devices registered yet."}</p></div></div>:<div className="qs-scroll-region min-h-0 flex-1 divide-y divide-border">{rows.map(row=>{
            ...
            return <div key={row.id} className="flex flex-col gap-2 p-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-start gap-2">
                <span className={cn("grid size-8 shrink-0 place-items-center rounded-[9px]",isOnline?"bg-emerald-500/10 text-emerald-600":"bg-muted text-muted-foreground")}>{isOnline?<Wifi className="size-4"/>:<WifiOff className="size-4"/>}</span>
                <div className="min-w-0"><div className="flex flex-wrap items-center gap-1.5"><strong className="text-xs">{row.name}</strong><span className={cn("rounded-full px-2 py-0.5 text-[8px] font-bold",isOnline?"bg-emerald-500/10 text-emerald-700":row.is_active?"bg-slate-500/10 text-slate-600":"bg-red-500/10 text-red-600")}>{isOnline?(ar?"متصل":"Online"):row.is_active?(ar?"غير متصل":"Offline"):(ar?"ملغي":"Revoked")}</span>{isCurrent?<span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[8px] font-bold text-blue-700">{ar?"هذا الجهاز":"This device"}</span>:null}</div><p className="mt-0.5 text-[9px] uppercase tracking-wide text-muted-foreground">{row.device_type}{row.kitchen_station_id?` · ${stationMap.get(row.kitchen_station_id)??"Station"}`:""} · {row.token_prefix}••••</p><p className="mt-0.5 text-[9px] text-muted-foreground">{row.last_seen_at?(ar?"آخر نبضة: ":"Last seen: ")+new Date(row.last_seen_at).toLocaleString(ar?"ar-JO":"en-JO"):(ar?"لم يرسل نبضة بعد":"No heartbeat yet")}{row.last_route?` · ${row.last_route}`:""}</p></div>
              </div>
              {row.is_active?<Button size="sm" variant="outline" className="text-red-600" disabled={revoke.isPending} onClick={()=>revoke.mutate(row.id)}><Unplug className="size-3"/>{ar?"إلغاء":"Revoke"}</Button>:null}
            </div>;
          })}</div>}
        </article>

        <article className="qs-card flex min-h-0 flex-col overflow-hidden">
          <div className="border-b border-border p-3"><div className="flex items-center gap-2"><Printer className="size-4 text-[#ff5a0a]"/><h2 className="qs-section-title">{ar?"طابعات المطبخ":"Kitchen printers"}</h2></div><p className="mt-0.5 text-[10px] text-muted-foreground">{ar?"تعريفات الطباعة المرتبطة بمحطات KDS.":"Printer definitions and KDS station mappings."}</p></div>
          {printers.isPending?<div className="p-4"><Skeleton className="h-40 rounded-xl"/></div>:printerRows.length===0?<div className="grid min-h-0 flex-1 place-items-center p-4 text-center"><div><Printer className="mx-auto size-8 text-muted-foreground"/><p className="mt-2 text-xs text-muted-foreground">{ar?"لا توجد طابعات معرفة.":"No printers configured."}</p></div></div>:<div className="qs-scroll-region min-h-0 flex-1 space-y-2 p-2.5">{printerRows.map(row=><div key={row.id} className="rounded-[10px] border border-border bg-muted/15 p-3"><div className="flex items-start justify-between gap-2"><span className="grid size-8 place-items-center rounded-[9px] bg-muted"><Printer className="size-3.5"/></span><span className={cn("rounded-full px-2 py-0.5 text-[8px] font-bold",row.is_active?"bg-emerald-500/10 text-emerald-700":"bg-muted text-muted-foreground")}>{row.is_active?(ar?"مفعلة":"Active"):(ar?"معطلة":"Disabled")}</span></div><strong className="mt-2 block text-xs">{row.name}</strong><p className="mt-0.5 text-[9px] text-muted-foreground">{row.provider} · {row.purpose}{row.kitchen_station_id?` · ${stationMap.get(row.kitchen_station_id)??"Station"}`:""}</p>{row.endpoint?<p className="mt-0.5 truncate text-[9px] text-muted-foreground">{row.endpoint}</p>:null}</div>)}</div>}
        </article>
      </section>
    </main>
  </div>;
