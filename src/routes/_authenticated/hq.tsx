...
function HQPage(){
  ...
  return <div className="min-h-dvh bg-background"><AppHeader title="HQ"/><main className="qs-page space-y-5">
    <section className="overflow-hidden rounded-[28px] border border-border bg-card shadow-sm"><div className="p-6 sm:p-8"><div className="inline-flex items-center gap-2 rounded-full bg-orange-500/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[.16em] text-[#ff5a0a]"><Building2 className="size-3.5"/>{ar?"قيادة متعددة الفروع":"Multi-location command"}</div><h1 className="mt-4 font-display text-3xl font-bold tracking-[-.04em] sm:text-4xl">{ar?"كل فروعك في شاشة واحدة":"Every location in one portfolio"}</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{ar?"قارن المبيعات والطلبات والاستثناءات التشغيلية عبر الفروع بدون خلط بيانات المستأجرين.":"Compare sales, orders and operating exceptions across locations while preserving tenant boundaries."}</p></div></section>
    ...
    {query.isPending?<Skeleton className="h-80 rounded-2xl"/>:query.isError?<section className="qs-card p-5 text-sm text-destructive">{humanError(query.error,lang)}</section>:<>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Kpi icon={Building2} label={ar?"الفروع":"Locations"} value={formatNumber(rows.length,lang)}/><Kpi icon={ReceiptText} label={ar?"طلبات اليوم":"Orders today"} value={formatNumber(totalOrders,lang)}/><Kpi icon={Coins} label={ar?"مبيعات مدفوعة اليوم":"Paid sales today"} value={mixed?(ar?"عملات متعددة":"Multiple currencies"):formatMoney(totalSales,baseCurrency,lang)}/><Kpi icon={Package} label={ar?"استثناءات":"Exceptions"} value={formatNumber(low+unpaid,lang)} tone={low+unpaid?"warning":undefined}/></section>
