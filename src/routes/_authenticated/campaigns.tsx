    ...
    <main className="qs-page qs-compact-page space-y-4">
      ...
      <section className="grid gap-3 xl:grid-cols-[minmax(0,.9fr)_minmax(0,1.1fr)]">
        <article className="qs-card p-4">
          <h2 className="font-display text-xl font-bold">{ar?"إنشاء حملة":"Create campaign"}</h2>
          <div className="mt-4 space-y-4">
            <Input value={name} onChange={e=>setName(e.target.value)} placeholder={ar?"اسم الحملة":"Campaign name"}/>
            <div className="grid grid-cols-3 gap-2">{(["sms","whatsapp","email"] as const).map(value=><button key={value} type="button" onClick={()=>{setChannel(value);setPreview(null);}} className={cn("rounded-xl border p-3 text-xs font-bold capitalize",channel===value?"border-[#ff5a0a] bg-orange-500/10 text-[#ff5a0a]":"border-border")}>{value==="email"?<Mail className="mx-auto mb-1 size-4"/>:<MessageSquareText className="mx-auto mb-1 size-4"/>}{value}</button>)}</div>
            <label className="block text-xs font-semibold">{ar?"الشريحة":"Segment"}<select value={segment} onChange={e=>{setSegment(e.target.value);setPreview(null);}} className="mt-2 h-11 w-full rounded-xl border border-input bg-background px-3 text-sm">{SEGMENTS.map(([value,en,arabic])=><option key={value} value={value}>{ar?arabic:en}</option>)}</select></label>
            ...
            <Textarea rows={5} value={message} onChange={e=>setMessage(e.target.value)} maxLength={2000} placeholder={ar?"الرسالة — يمكنك استخدام {{name}}":"Message — you can use {{name}}"}/>
        ...
        <article className="qs-card overflow-hidden">
          ...
          {campaigns.isPending?<div className="p-5"><Skeleton className="h-80 rounded-xl"/></div>:(campaigns.data??[]).length===0?<div className="grid min-h-[240px] place-items-center p-6 text-center"><div><Megaphone className="mx-auto size-9 text-muted-foreground"/><p className="mt-3 text-sm text-muted-foreground">{ar?"لا توجد حملات بعد.":"No campaigns yet."}</p></div></div>:<div className="divide-y divide-border">{(campaigns.data??[]).map(row=><div key={row.id} className="p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><strong>{row.name}</strong><Status status={row.status}/></div><p className="mt-1 text-xs text-muted-foreground">{row.channel.toUpperCase()} · {row.segment_type.replaceAll("_"," ")} · {row.scheduled_at?new Date(row.scheduled_at).toLocaleString(ar?"ar-JO":"en-US"):"—"}</p></div>{["draft","scheduled","processing","blocked"].includes(row.status)?<Button size="sm" variant="outline" onClick={()=>cancel.mutate(row.id)}><XCircle className="size-3"/>{ar?"إلغاء":"Cancel"}</Button>:null}</div><div className="mt-3 grid grid-cols-4 gap-2 text-center text-[10px]"><Count label={ar?"الجمهور":"Audience"} value={row.recipient_count}/><Count label={ar?"أرسل":"Sent"} value={row.sent_count}/><Count label={ar?"فشل":"Failed"} value={row.failed_count}/><Count label={ar?"تخطي":"Skipped"} value={row.skipped_count}/></div>{row.last_error?<p className="mt-3 rounded-lg bg-amber-500/10 p-2 text-xs text-amber-800">{row.last_error}</p>:null}</div>)}</div>}
        </article>
      </section>
