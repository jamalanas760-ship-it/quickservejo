...
function CampaignsPage() {
  ...
  return <div className="min-h-dvh bg-background">
    ...
    <main className="qs-page qs-compact-page qs-viewport-page">
      ...
      <section className="qs-viewport-fill grid min-h-0 gap-2 overflow-hidden xl:grid-cols-[minmax(300px,.82fr)_minmax(0,1.08fr)_minmax(300px,.9fr)]">
        <article className="qs-card qs-scroll-region min-h-0 p-3">
          ...
          <div className="mt-3 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">{(["sms","whatsapp","email"] as const).map(value=><button key={value} type="button" onClick={()=>{setChannel(value);setPreview(null);}} className={cn("rounded-[10px] border p-2.5 text-[11px] font-bold capitalize",channel===value?"border-[#e85d2a] bg-orange-500/10 text-[#e85d2a]":"border-border")}>{value==="email"?<Mail className="mx-auto mb-1 size-4"/>:<MessageSquareText className="mx-auto mb-1 size-4"/>}{value}</button>)}</div>
        ...
        <article className="qs-card flex min-h-0 flex-col overflow-hidden">
          <div className="border-b border-border p-3"><h2 className="font-display text-base font-bold">{ar?"سجل الحملات":"Campaign history"}</h2><p className="mt-1 text-xs text-muted-foreground">{ar?"الحالة والأرقام من طابور التسليم الحقيقي.":"Statuses and counts come from the real delivery queue."}</p></div>
          {campaigns.isPending?<div className="p-5"><Skeleton className="h-80 rounded-xl"/></div>:(campaigns.data??[]).length===0?<div className="grid min-h-[160px] place-items-center p-4 text-center"><div><Megaphone className="mx-auto size-9 text-muted-foreground"/><p className="mt-3 text-sm text-muted-foreground">{ar?"لا توجد حملات بعد.":"No campaigns yet."}</p></div></div>:<div className="qs-scroll-region min-h-0 flex-1 divide-y divide-border">{(campaigns.data??[]).map(row=><div key={row.id} className="p-3.5"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><strong>{row.name}</strong><Status status={row.status}/></div><p className="mt-1 text-xs text-muted-foreground">{row.channel.toUpperCase()} · {row.segment_type.replaceAll("_"," ")} · {row.scheduled_at?new Date(row.scheduled_at).toLocaleString(ar?"ar-JO":"en-US"):"—"}</p></div>{["draft","scheduled","processing","blocked"].includes(row.status)?<Button size="sm" variant="outline" onClick={()=>cancel.mutate(row.id)}><XCircle className="size-3"/>{ar?"إلغاء":"Cancel"}</Button>:null}</div><div className="mt-3 grid grid-cols-4 gap-2 text-center text-[10px]"><Count label={ar?"الجمهور":"Audience"} value={row.recipient_count}/><Count label={ar?"أرسل":"Sent"} value={row.sent_count}/><Count label={ar?"فشل":"Failed"} value={row.failed_count}/><Count label={ar?"تخطي":"Skipped"} value={row.skipped_count}/></div>{row.last_error?<p className="mt-3 rounded-lg bg-amber-500/10 p-2 text-xs text-amber-800">{row.last_error}</p>:null}</div>)}</div>}
        </article>
        <aside className="qs-card qs-scroll-region min-h-0 overflow-hidden p-3"><CrmAutomationPanel restaurantId={rid} compact /></aside>
      </section>
    </main>
  </div>;
}
