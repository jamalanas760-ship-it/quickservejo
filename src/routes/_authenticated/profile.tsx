      ...
      <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)] xl:items-start">
        <aside className="xl:sticky xl:top-24">
          ...
          <div className="mt-3 hidden rounded-2xl border border-border bg-muted/20 p-4 xl:block">
            ...
            <p className="mt-2 text-[11px] leading-5 text-muted-foreground">{ar ? "بيانات الحساب الأساسية للعرض فقط، بينما تفضيلاتك وإعدادات المطعم محفوظة بشكل مستقل." : "Core account details are read-only while your preferences and restaurant settings are saved independently."}</p>
          </div>
        </aside>
...
function PersonalSection({ ar, lang, rid, displayName, email, roleLabel, restaurantName, createdAt }: { ar: boolean; lang: "ar" | "en"; rid: string | null; displayName: string; email: string; roleLabel: string; restaurantName: string; createdAt: string | undefined }) {
  return <div className="space-y-5">
    ...
    <section className="qs-card overflow-hidden">
      <div className="border-b border-border px-5 py-4 sm:px-6"><h2 className="text-sm font-bold">{ar ? "تفاصيل الحساب" : "Account details"}</h2><p className="mt-1 text-xs text-muted-foreground">{ar ? "هذه البيانات مرتبطة بحسابك وصلاحياتك الحالية." : "These details reflect your current account and access scope."}</p></div>
      <div className="grid gap-px bg-border sm:grid-cols-2">
        <InfoTile icon={<User className="size-4" />} label={ar ? "الاسم الكامل" : "Full name"} value={displayName} />
