...
import { loadDinerMenu, placePublicOrder, type CartLine, type DinerItem, type PlacedOrder } from "@/lib/diner";

const searchSchema = z.object({ t: z.string().optional() });

export const Route = createFileRoute("/m/$slug")({
  validateSearch: searchSchema,
  head: () => ({ meta: [{ title: "Menu" }, { name: "description", content: "Order directly from the restaurant menu." }] }),
...
function PdfOrderPage() {
  ...
  const menu = useQuery({ queryKey: ["pdf-diner", slug, qrToken ?? null], queryFn: () => loadDinerMenu(slug, qrToken ?? null), retry: 1, staleTime: 60_000, gcTime: 10 * 60_000, refetchOnWindowFocus: false });
  ...
  // Deliberately no QuickServe splash/loading screen: the QR destination is a
  // customer ordering surface and should reveal the menu as soon as data arrives.
  if (menu.isPending) return <div className="min-h-screen bg-white" aria-label="Loading menu" />;
  if (menu.isError || !menu.data) return <div className="grid min-h-screen place-items-center bg-background p-6 text-center"><div><h1 className="text-xl font-bold">Menu unavailable</h1><p className="mt-2 text-sm text-muted-foreground">{humanError(menu.error)}</p></div></div>;

  const { restaurant, pdfMenu } = menu.data;
  if (!pdfMenu) return <div className="grid min-h-screen place-items-center p-6 text-center"><div><h1 className="text-xl font-bold">Menu unavailable</h1><p className="mt-2 text-sm text-muted-foreground">This restaurant has no active PDF menu.</p><Button asChild className="mt-4"><Link to="/r/$slug" params={{ slug }}>Open menu</Link></Button></div></div>;

  // A QR token is sufficient to enable the ordering UI. The database RPC
  // remains the final authority and validates the token when the order is sent.
  const canOrder = Boolean(qrToken && menu.data.settings?.enable_orders !== false);
  ...
  async function submitOrder() {
    ...
    try {
      ...
      setPlaced(result); setCart([]); setNotes(""); setCartOpen(false);
    } catch (error) {
  ...
  return <div className="min-h-screen bg-white pb-24">
    <header className="sticky top-0 z-40 border-b bg-white/95 px-2 py-2 backdrop-blur-xl">
      <div className="mx-auto flex max-w-4xl items-center gap-2">
        <p className="min-w-0 flex-1 truncate text-sm font-bold">{restaurant.name}</p>
        <Button size="sm" variant="ghost" onClick={toggleLang}>{lang === "ar" ? "EN" : "ع"}</Button>
        {canOrder && cartCount > 0 ? <Button size="sm" onClick={() => setCartOpen(true)}><ShoppingBag className="size-4"/> {cartCount}</Button> : null}
      </div>
    </header>

    <main><PdfMenuViewer url={pdfMenu.url} parts={pdfMenu.parts} pageCount={pdfMenu.pageCount} links={pdfMenu.links} items={menu.data.items} onSelect={(item) => canOrder ? setDetail(item) : toast.error(lang === "ar" ? "الطلب غير متاح" : "Ordering is unavailable")} cartCount={cartCount} onCart={() => setCartOpen(true)} showCart={canOrder}/></main>

    {canOrder && cartCount > 0 ? <div className="safe-bottom fixed inset-x-0 bottom-0 z-50 border-t bg-white/95 p-3 backdrop-blur"><div className="mx-auto max-w-4xl"><Button className="w-full" onClick={() => setCartOpen(true)}><ShoppingBag className="size-4"/> {lang === "ar" ? "عرض السلة" : "View cart"} ({cartCount}) · {formatMoney(total, currency, lang)}</Button></div></div> : null}
