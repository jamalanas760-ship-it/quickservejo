...
function PdfOrderPage() {
  ...
  if (menu.isPending) {
    return <div className="min-h-screen bg-white" aria-label="Loading menu" />;
  }
  ...
  const canOrder =
    Boolean(qrToken) && menu.data.settings?.enable_orders !== false;
  ...
  const service = menu.data.settings?.enable_service_charge
    ? subtotal * (restaurant.service_charge / 100)
    : 0;
  ...
  function addLine(
    ...
    const unitPrice =
      item.price +
      modifiers.reduce((sum, modifier) => sum + modifier.price_delta, 0);
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
        ...
        <PdfMenuViewer
          ...
          onSelect={(item) => {
            if (canOrder) {
              ...
              toast.error(
                lang === "ar" ? "الطلب غير متاح" : "Ordering is unavailable",
              );
            }
          }}
          cartCount={cartCount}
      ...
      {canOrder && cartCount > 0 ? (
        <div className="safe-bottom fixed inset-x-0 bottom-0 z-50 border-t bg-white/95 p-3 backdrop-blur">
          <div className="mx-auto max-w-4xl">
            <Button className="w-full" onClick={() => setCartOpen(true)}>
              <ShoppingBag className="size-4" />
              {lang === "ar" ? "عرض السلة" : "View cart"} ({cartCount}) · {formatMoney(total, currency, lang)}
            </Button>
          </div>
        </div>
      ) : null}
      ...
      <Sheet open={cartOpen} onOpenChange={setCartOpen}>
        <SheetContent side="bottom" className="max-h-[88vh] overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{lang === "ar" ? "طلبك" : "Your order"}</SheetTitle>
          </SheetHeader>
          <div className="space-y-3 p-4">
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setCart((previous) =>
                        previous.filter((item) => item.key !== line.key),
                      )
                    }
                  >
            ...
            <div className="space-y-1.5">
              <Label>{lang === "ar" ? "ملاحظات الطلب" : "Order notes"}</Label>
              <Textarea
                value={notes}
                onChange={(event) => setNotes(event.target.value)}
                rows={2}
                placeholder={
                  lang === "ar" ? "مثال: بدون بصل" : "Example: no onions"
                }
              />
            </div>
            ...
            <Button
              className="w-full"
              disabled={busy || !cart.length || !canOrder}
      ...
      <Dialog
        open={placed !== null}
        onOpenChange={(open) => !open && setPlaced(null)}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
...
function PdfItemSheet({
  ...
  const missingRequired = (item?.groups ?? []).some((group) => {
    ...
    return selectedCount < Math.max(1, group.min_selection);
  });
        ...
        {item ? (
          <div className="space-y-5 p-4">
            <div className="text-lg font-bold">
              {formatMoney(item.price + extra, currency, lang)}
            </div>
